/* (c) Copyright 2005-2008, The Rockefeller University *11116* */
/* $Id: nelmeadf.h 59 2017-01-13 20:34:16Z  $ */
/***********************************************************************
*                           Header File for                            *
...
typedef float nmxyf;       /* Type of simplex and solutions */

struct nmhistf {           /* Data returned in history array */
   nmxyf    y;                /* Y value at this iteration */
   nmxyf    ybest;            /* Best y in current simplex */
   nmxyf    yworst;           /* Worst y in current simplex */
   nmxyf    yglmin;           /* Best ever y value */
...
*  (Near the largest representable float in IEEE arithmetic).  */
#define NM_SING      1.0E36
...
void nmprtxf(nmxyf *x, int N); /* Error print routine */
struct nmglblf {
   struct nmhistf *phist;  /* Ptr to history array */
   nmxyf (*ufn)(nmxyf *x, void *usrd); /* Ptr to user function */
   nmxyf  *nmasx;          /* Ptr to simplex of N+1 N-dim vertices */
   nmxyf  *nmasy;          /* Ptr to vector of N+1 function values
                           *  at the vertices of x */
   nmxyf  *nmasxb;         /* Ptr to x vector at best y */
   char   *msgid;          /* Ptr to id info for warning messages */
   nmxyf  nmasyb;          /* Best value of y found so far */
   nmxyf  nmasrr;          /* Reflection ratio (normally 1.0) */
   nmxyf  nmasxr;          /* Expansion ratio (normally 2.0) */
   nmxyf  nmascr;          /* Contraction ratio (normally 0.5) */
   nmxyf  nmassr;          /* Shrinkage ratio (normally 0.5) */
   nmxyf  nmasar;          /* Avoidance ratio (0 if kavoid = 0) */
...
nmxyf nminitf(struct nmglblf *nmG, void *usrd, char *msgid, int options);
