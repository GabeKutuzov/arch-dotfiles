...
/* $Id: axlog.c 34 2017-09-15 18:13:10Z  $ */
/***********************************************************************
*                               axlog()                                *
*                                                                      *
*  SYNOPSIS:                                                           *
...
*  Rev, 05/02/17, GNR - Avoid minor ticks off end of axis              *
*  Rev, 05/02/17, GNR - Modify for PARn nodes (no RK or RKC)           *
***********************************************************************/

#include "glu.h"                 /* (Includes math.h etc.) */
#include "plots.h"
#include "rksubs.h"
...
void axlog(float x, float y, const char *label, float axlen,
   ...
   if (_RKG.s.MFActive && axlen > 0.0) {
      ...
      int   iavm;                /* abs(vmult) */
      int   bcdoic;              /* Format code for bcdoutPn() */
      int   nl,nl0 = 0;          /* Number of characters to plot */
      ...
      if (deltat > 0.0) {
         ...
         if (vmult >= 0.0) {
            ...
            if (height > 0.0) {     /* Prepare for tick labels */
               ty = loff + stv*height;
               loff += stk*height;  /* Y offset for axis label */
               wbcd = 0.5*ASPECT*height;
               if (++nd < 0) nd = 0;
               if (ktk & AX_TKEXP) {         /* Base to exponent */
                  iavm = abs((int)vmult);
                  wbcdwtPn(&iavm, printme, RK_LFTJ|RK_IORF|RK_NINT|3);
                  printme[RKPn.length+1] = 0x12;   /* Superscript */
                  tx = SUPEROFF*height;
                  ty += stv*tx;
                  loff += stk*tx;
                  vmult = (vmult >= 1.0) ? 1.0 : -1.0;
                  bcdoic = RK_LFTJ|RK_IORF|3;
                  prntval = printme + (nl0 = RKPn.length + 2);
                  }
               else if (ktk & AX_TKEFMT) {
                  bcdoic = (nd+1)<<RK_DS | RK_EFMT | (MXTVLEN-1);
                  prntval = printme; }
               else {
                  bcdoic = (nd+1)<<RK_DS|RK_UFLW|RK_IORF|(MXTVLEN-1);
                  prntval = printme; }
               }
            else
               nd = -1;
         ...
         for (tval=firstv,ux=firstt; ux<=axlen;
            ...
            if (nd >= 0) {                /* Plot tick value */
               bcdoutPn(bcdoic, prntval, (double)tval);
               nl = nl0 + RKPn.length + 1;
               if (ktk & AX_TKEXP) {
                  /* Note:  It is not necessary to cancel the
                  *  superscript because mfdraw forgets it at
                  *  the end of the current text command.  */
                  tval += vmult;
                  tx = ux - wbcd*(float)(nl-1);
                  }
               else {
                  tval *= vmult;
                  tx = ux - wbcd*(float)(nl);
