...
/* $Id: bpause.c 31 2017-04-13 21:51:22Z  $ */
/***********************************************************************
*                              bpause()                                *
*                                                                      *
*  SYNOPSIS:                                                           *
...
*                                                                      *
*  Action:  If mfsr returns a code that requires user program action,  *
*     and that action can be done here, it is done here and the bit is *
*     cleared.  Any requested movie exposures are also obtained here   *
*     (obsolete).  Remaining ackbutt.mfdb.intxeq codes are returned.   *
************************************************************************
...
int bpause(void) {
   ...
   *  Mfdraw takes care of other buttons pressed by user */
   if (_RKG.s.MFMode & SGX) {
      ...
      *  plan to work with multiple sets of buttons is effectuated. */
      if (_RKG.ackb.mfdb.movie_mode)
         _RKG.movie_mode = _RKG.ackb.mfdb.movie_mode;
      }
