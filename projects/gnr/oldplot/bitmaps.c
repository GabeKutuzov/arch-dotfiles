/* (c) Copyright 2006-2017, The Rockefeller University *11114* */
/* $Id: bitmaps.c 34 2017-09-15 18:13:10Z  $ */
/***********************************************************************
*                              bitmaps()                               *
*                                                                      *
*  SYNOPSIS:                                                           *
*     void bitmaps(const byte *array, int rowlen, int colht,           *
*        float xc, float yc, float bwd, float bht, int xoffset,        *
*        int yoffset, int iwd, int iht, int type, int mode)            *
*                                                                      *
*  DESCRIPTION:                                                        *
*     Plots an image on the screen in the specified location and size. *
*     The bitmap can be scaled to a specified size in inches.          *
*     This routine is the same as bitmap() except for the addition     *
*     of the bwd, bht arguments.                                       *
*                                                                      *
*  ARGUMENTS:                                                          *
...
*     (i.e. a map of 'rowlen' pixels per row and 'colht' pixels per    *
*     column) in inches on the same scale as that of all the vector    *
*     plotting functions that specify coordinates in inches.  The      *
*     rendering implementation is expected to scale the bitmap as      *
*     required--the unscaled data are stored in the metafile.          *
*                                                                      *
...
*     None.                                                            *
************************************************************************
*  New program, 10/26/06, GNR - Made from a copy of bitmap.c           *
*  Rev, 06/03/07, GNR - Add BM_C8, BM_C16, BM_C24 modes.               *
...
#include "glu.h"
#include "rksubs.h"
#define MAX_BM_HEADER 48         /* Max BitMap header size plus
                                 *  one for LF at end of pane */

void bitmaps(const byte *array, int rowlen, int colht, float xc,
...
/* Set up image extraction according to map type */
   ...
   case BM_C8:                /* Old SunOS/NCUBE 8-bit color map */
      ...
      xoffs = xoffset;
      break;
   case BM_GS16:              /* 16-bit gray scale */
   case BM_C16:
      rlen = iwd + iwd;
   ...
   if (iht > 0) {
      stride = rowbytes;
      yoffs = colht - yoffset;
      prow = array + (NoExtract ? 0 : rowbytes*yoffs);
      }
   else {
      iht = abs(iht);
      stride = -rowbytes;
...
/* Generate segment header */
      ...
      *_RKG.MFCurrPos++ = mode + '0';
      ...
      i2a(_RKG.MFCurrPos,(int)(bwd*1000.0),5,DECENCD);
      i2a(_RKG.MFCurrPos,(int)(bht*1000.0),5,DECENCD);
