...
/* $Id: d3grp2.c 32 2010-10-19 15:53:04Z  $ */
/***********************************************************************
*                             CNS Program                              *
...
*                       CONNTYPE, CELLTYPE decay, OPT=P sets hesam,    *
...
*  The enum values must match the corresponding cc2 entries! */
   ...
   long inseed = 0;           /* Global default random number seed */
...
*  link it to the tree structure, and initialize it.  */
         ...
         ignore = IGN_ReadAll;
         *next_REPBLOCK = ir = (struct REPBLOCK *)
            allocpcv(Static, 1, IXstr_REPBLOCK, "REPBLOCK structure");
...
*---------------------------------------------------------------------*/
      ...
      case AEIF_CARD:
         ...
         *  bring in the defaults */
         if (!il->prfi) { il->prfi = allocprv(Static, 1,
...
*---------------------------------------------------------------------*/
      ...
      {  rlnm  tsrcnm;           /* Temp source name */
         ...
         if (cc2index == GCONN_CARD) {    /* New GCONN format */
            if (readrlnm(&tsrcnm, il) != REPSRC) cryout(RK_E1,
               "0***GCONN INPUT MUST BE A REGION-LAYER NAME.",
               RK_LN2, NULL);
            }
         else {                           /* Old INHIB format */
            setrlnm(&tsrcnm, ir->sname, il->lname);
            cdscan(RK.last, 1, SCANLEN, RK_WDSKIP);
            d3rlnm(&tsrcnm);
            }
...
*  and initialize it  */
         ...
         *next_INHIBBLK = ib = (struct INHIBBLK *)
            allocpcv(Static, 1, IXstr_INHIBBLK, "INHIBBLK structure");
...
*---------------------------------------------------------------------*/
      ...
      case CONNTYPE_CARD:
      {  long lnc, tmseed = inseed;
         int  ith,ithe,kith;
         static char *thlim[2] = { "2", "256" };

         inseed += 99991L;
         if (ignore >= IGN_Celltype) goto IGNORE_CARD;
         if (!il) goto NO_CELL_CARD;
         inform("(SW1,2XIL)", &lnc, NULL);
         if (lnc <= 0) {
...
/* Allocate storage for a new CONNTYPE block, link it into the
*  tree structure, move any deferred thresholds into LCnD, and
*  initialize the CONNTYPE, including moving in RP0->LCnD. */
         ...
         *next_CONNTYPE = ix = (struct CONNTYPE *)allocpcv(Static,
         ...
         ix->cnsrctyp = (byte)readrlnm(&ix->srcnm, il);
         kith = ix->cnsrctyp == REPSRC && !(RP->compat & OLDRANGE);
         ithe = (int)RP0->ndfth;
         for (ith=0; ith<ithe; ++ith) {
            si32 tth = RP0->dfthr[ith].s14thr;
            /* Round if truncating to S7 */
            if (kith) tth = SRA(tth+64, Sr2mV);
            if (labs(tth) >= S16) cryout(RK_E1,
               "0***MAGNITUDE OF A THRESHOLD PARAMETER FROM AN "
               "EARLIER CONTROL CARD EXCEEDS ", RK_LN2,
               thlim[kith], RK_CCL, NULL);
            *RP0->dfthr[ith].pthr = (short)tth;
            }
         cndflt(il, ix);
         ix->nc = lnc;
