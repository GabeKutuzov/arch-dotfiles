/* (c) Copyright 1997-2007, Neurosciences Research Foundation, Inc. */
/* $Id: d3rpsi.c 48 2012-04-23 18:40:15Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                              d3rpsi.c                                *
*                                                                      *
...
#error "jcell must be initialized to 0,...,CELLS_PER_LINE-1 "
   ...
   long icell;                /* First cell on current line */
   long lncells;              /* Actual number of cells per line */
   long ncr;                  /* Number of cells remaining */
   long si;                   /* s(i) value aligned for printing */
   ...
   for (idly = kdly; idly >= 0; idly--) {
      ...
      for (icell=0; icell<il->nelt; icell+=lncells) {
         ncells = (int)min(lncells, ncr);
         ibcdwt(RK_IORF+7, outln, icell);
         pol = outln + 8; pole = pol + itlen*ncells;
         for ( ; pol<pole; psi+=il->lsp,pol+=itlen) {
            d3gts2(si, psi);
            sconvrt(pol, "(OB7/14IL8.<4,RUC3)",
