/* (c) Copyright 1994-2011 Neurosciences Research Foundation, Inc. */
/* $Id: d3g1g3.c 51 2012-05-24 20:53:36Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                              d3g1g3.c                                *
*                                                                      *
...
void getcolor(void) {
   ...
   while (kwret = kwscan(&ic, /* Assignment intended */
         ...
         "D1BARS%VA"      qCMC, RP->colors[CI_BAR],
         "HITHIT%VA"      qCMC, RP->colors[CI_HPP],
         "HITNOHIT%VA"    qCMC, RP->colors[CI_HPM],
         "NOHITHIT%VA"    qCMC, RP->colors[CI_HMP],
         "NOHITNOHIT%VA"  qCMC, RP->colors[CI_HMM],
...
*     DECAY=omega                                                      *
*     DECAY=(omega)                                                    *
*     DECAY=(omega,<EXPONENTIAL | LIMITING | SATURATING>)              *
*     DECAY=(omega, SATURATING, halfeff)                               *
*  (Any of the method keywords can as usual be abbreviated by any      *
*  unique leading substring.  The halfeff parameter is restored to     *
*  its default value iff a method keyword is entered.)  The results    *
*  are stored in a DECAYDEF structure, to which the only argument      *
*  must point.  On entry, the control card has been read and is in     *
*  RK.last and DECAY is the next positional parameter or the DECAY     *
...
#define NDCYKEYS 3
   static char *gdcykeys[NDCYKEYS] =
      {"EXPONENTIAL", "LIMITING", "SATURATING"};
   static float gdcyvals[NDCYKEYS] =
      {DCYEXP,        DCYLIM,     DCYSAT};

   float halfeff;          /* Input half-effective value */
   int kdecay;             /* Method keyword index */
   char lea[SCANLEN1];     /* Space for scanning */

   /* Deal with omega parameter */
   scanck(lea, RK_REQFLD,
      ~(RK_COMMA|RK_BLANK|RK_INPARENS|RK_RTPAREN));
   pdcd->omega = ibcdin(FBod*RK_BSCL+RK_GFP+RK_CTST+RK_QPOS+
      RK_IORF+RK.length, lea);
   if ((RK.scancode & (RK_INPARENS|RK_RTPAREN)) != RK_INPARENS)
      return;

   /* Deal with method keyword.  RK.iexit gets set by match() if
   *  a bad string is entered, so we return at end of parentheses.
   *  The fact that the method is encoded in the half-effectiveness
   *  field is the reason why the default is reset by a new keyword.
   */
   if ((kdecay = match(0, RK_MREQF, ~(RK_COMMA|RK_RTPAREN),
         RK_INPARENS, gdcykeys, NDCYKEYS)) > 0)
      pdcd->hesam = gdcyvals[kdecay - 1];

   /* Handle halfeff parameter if SATURATING
   *  (assumed to be last key in list above)  */
   if (kdecay == NDCYKEYS) {
      pdcd->hesam = RP0->satdcynum; /* Enter default hesam */
      if (!(RK.scancode & RK_RTPAREN)) {
         scanck(lea, RK_REQFLD,
            ~(RK_COMMA|RK_BLANK|RK_INPARENS|RK_RTPAREN));
         halfeff = bcdin(RK_IORF+RK_SNGL+
            RK_CTST+RK_QPOS+RK.length, lea);
         if (halfeff > 0.0) pdcd->hesam = RP0->satdcynum/halfeff;
         else ermark(RK_NUMBERR);
         } /* End scan for halfeff */
      } /* End SATURATING */

   /* Perform final check that we got to end of parens */
   if (!(RK.scancode & RK_RTPAREN)) ermark(RK_TOOMANY);
   } /* End getdecay() */
...
*  Rev, 06/16/08, GNR - Add CSELPLOT, smshrink                         *
***********************************************************************/
...
void getijpl(enum KIJPlot kijpl) {
   ...
   ui32   ic;                 /* kwscan() field rec codes */
   int    kwret;              /* kwscan() return code     */
   ...
   *  on scale (S-1) as would be needed for L2S7 variables.  */
   ...
   static char *cmmxopts[] = { "MXCIJ%B14IH", "MXCIJ0%B14IH",
      ...
      "MXSJ%B7/14IH$mV", "MXESJ%B7/14IH$mV", NULL };
...
*  one and initialize it with default values.  */
   ...
   *  cell list is entered, pc->cmclid will remain zero.  */
   pc = *ppc = callocv(1, sizeof(struct IJPLDEF),
...
GotIJPLDEF:
   ...
   while (kwret = kwscan(&ic, /* Assignment intended */
         ...
         "SMSHRINK%V<1.0F",      &pc->smshrink,
         "SRCNGX%V>IL",          &pc->srcncx,   /* For compatibility */
         "SRCNCX%V>IL",          &pc->srcncx,
         "TGTNCX%V>IL",          &pc->tgtncx,
         "OPT%KHNGJIATX2KPVOSM", &pc->cmopt,
         "LABELTEXT%TH" qLTF,    &pc->cmltxt,
         "LTEXT%TH" qLTF,        &pc->cmltxt,
         "LABELCOLOR%VA" qCMC,   pc->cmltc,
         "LCOLOR%VA" qCMC,       pc->cmltc,
         "DISABLE%S1C",          &pc->kcmpldis,
         "ENABLE%S0C",           &pc->kcmpldis,
      ...
      case 1:              /* CELLS option */
         ...
         *  leave the IJPLDEF and by default plot all cells.  */
         if ((pclh = d3clst(pc->cmclid, CLKM_SINGLE)) != NULL)
      ...
      case 2:              /* CLIST option */
         ...
         if (pc->cmclid < 0) d3clrm(pc->cmclid);
         /* Read new cell list num.  If OFF key was entered, clear
         *  out the cmclid and all cells will be plotted.  */
         if (getclid(&pc->cmclid, &pc->cmclndx)) {
            pc->cmclid = 0;
            pc->cmclndx = 0;
            }
         break;
   ...
   if (bitcnt((byte *)&tjijpl, sizeof(tjijpl)) > 1)
      cryout(RK_E1, "0***USE ONLY ONE OF A,J,T OPTIONS.",
         RK_LN2, NULL);
   tjijpl = pc->cmopt & (CMSMAP|CMISIL);
   if (bitcnt((byte *)&tjijpl, sizeof(tjijpl)) > 1)
      cryout(RK_E1, "0***USE ONLY ONE OF I,S OPTIONS.",
         RK_LN2, NULL);
...
*                               getitp                                 *
*                                                                      *
*  Processes the IMGTOUCH card, which allows a touch pad in a BBD to   *
*  access items in an image.                                           *
...
*  This is allowed even in Group I--can't hurt anything.  */
   ...
   if (RK.scancode == (RK_INPARENS|RK_RTPAREN)) {
      iitp = ibcdin(RK_ZTST|STR_TO_INT, lea);
...
/* No index.  This is a new one.  Error if in Group III.  */
      ...
      *RP0->plvc = (struct VCELL *)pitp;
      ...
      pitp->vc.nvx = ibcdin(RK_ZTST|STR_TO_INT, lea);
      inform("(S,V>I,TH" qLTF ",A" qLSN ")",
         &pitp->vc.nvy, &pitp->hcamnm, pitp->vc.vcname, NULL);
...
/* Read keyword parameters common to Group I and Group III */
   ...
   kwscan(&ic,
         ...
         "SCLY%F",            &pitp->scly,
         "SCLA%F",            &pitp->scla,
         "TLA%RV>F", isgrp1,  &pitp->tla,
         "TLL%RV>F", isgrp1,  &pitp->tll,
   ...
   if (pitp->tll <= 0.0) pitp->tll = (float)pitp->vc.nvx;
   if (pitp->tla <= 0.0) pitp->tla = (float)pitp->vc.nvy;
   /* Use vcmin,vcmax to store edges of touchpad in image units */
   pitp->vc.vcmin = pitp->tll/(float)pitp->vc.nvx;
   pitp->vc.vcmax = pitp->tla/(float)pitp->vc.nvy;
   } /* End getitp() */
...
/* Print verification */
   ...
   if (kvfy) {
      convrt("(P4,'0VERIFY: '|#" qPHR ",(8B28IL9.6))",
         ppcf->bins, NULL);
...
*  This is allowed even in Group I--can't hurt anything.  */
   ...
   if (scan(lea, RK_REQFLD) & RK_ENDCARD)
      return;
   if (RK.scancode == (RK_INPARENS|RK_RTPAREN)) {
      iplb = ibcdin(RK_ZTST|STR_TO_INT, lea) - 1;
...
*  This is allowed even in Group I--can't hurt anything.  */
   ...
   if (scan(lea, RK_REQFLD) & RK_ENDCARD)
      return;
   if (RK.scancode == (RK_INPARENS|RK_RTPAREN)) {
      ipln = ibcdin(RK_ZTST|STR_TO_INT, lea) - 1;
...
#define MXFLDLEN ((SCANLEN > MXKEYLEN) ? SCANLEN : MXKEYLEN)
   ...
   *  as abbreviations of one of the SAVE card keys.  */
   ...
   enum svops { SVO_NONE=0, SVO_NETW, SVO_NEWS, SVO_CREAT,
      SVO_ROT, SVO_NOREP, SVO_REPL, SVO_RESP, SVO_SIMD,
      ...
      switch (smret) {
         ...
         case SVO_NEW:
            ...
            *  list--if not, d3clst will generate an error.  */
            scanagn();
            if (pgd->clid >= 0) pgd->clid = --RP0->nextclid;
            d3clst((int)pgd->clid, CLKM_INDIV);
            break;
...
#define SVSVI (GFSI|GFGLBL|GFVAL|GFOXY|GFOID|GFSNS|GFSID|GFUTV)
         ...
         case SVO_CLIST:      /* CLIST    */
            ...
            if (pgd->clid < 0) d3clrm(pgd->clid);
            if (getclid(&pgd->clid, NULL)) pgd->clid = 0;
            break;
            ...
            do {
               ...
               if (RK.scancode & RK_LFTPAREN) {
                  ...
                  if (!(RK.scancode & RK_RTPAREN)) {
                     ...
                     skip2end();
                     return; }
                  thgsnm = (txtid)ibcdin(RK_SNGL|RK_IORF|RK_QPOS|
                     RK_CTST|RK_ZTST + RK.length, lea);
                  }
               else {
...
*  intended scale.                                                     *
*                                                                      *
*  Note that the scales are stored as L2S8 values--when more precision *
*  is needed, the user should set the global scale with delta and use  *
...
*  or 8 values, so the warning has been moved to d3tchk.               *
*                                                                      *
*  Arguments:                                                          *
...
void getway(struct CNDFLT *pdd) {
   ...
   if (RK.scancode & RK_INPARENS) {
      ...
      scanagn();
      j = MAX_WAYSET_CODES;
      inform("(SV(NRB8IHV))", &j, pdd->wayset, NULL);
      ...
      if (RK.length >= MAX_WAYSET_CODES) ermark(RK_LENGERR);
         ...
         pdd->wayset[j] = (lea[j] & 1) ? (lea[j] & 2 ? -S8 : S8) : 0;
      pdd->nwayrd = j;
