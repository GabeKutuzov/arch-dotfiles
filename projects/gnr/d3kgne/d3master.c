...
/* $Id: d3master.c 51 2012-05-24 20:53:36Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                              g2master                                *
*                                                                      *
...
#define NMSUBS (sizeof(msubs)/sizeof(char *))
   ...
   imat = match(0, RK_MREQF, ~RK_COMMA, 0, msubs, NMSUBS);
   if (imat <= BAD_MATCH) {
      skip2end();
      return;
      }
...
*---------------------------------------------------------------------*/
   ...
   case MS_REP:
      ...
      kwscan(&ic,
         ...
         "RPTIMER%N" KWS_TIMNW,  &RP0->GRpD.jrptim,
...
*---------------------------------------------------------------------*/
   ...
   case MS_CT:
      while (kwret = kwscan(&ic, /* Assignment intended */
         ...
         "THAMP%B20/27IJ$mV",    &RP0->GCtD.stanh,
         "PT%B20/27IL$mV",       &RP0->GCtD.pt,
         "ST%B20/27IL$mV",       &RP0->GCtD.st,
         "NT%B20/27IL$mV",       &RP0->GCtD.nt,
         "HT%B20/27IL$mV",       &RP0->GCtD.ht,
         "CM%V>8B23UJ$pF",       &RP0->GCtD.Cm,
         "CPR%VF",               &RP0->GCtD.cpr,
         "CPT%B7/14IH$mV",       &RP0->GCtD.cpt,
         "CTOPT%KH" okctopt,     &RP0->GCtD.ctopt,
         "KCTP%KL" okctkrp,      &RP0->GCtD.kctp,
         "KETIMER%N" KWS_TIMNW,  &RP0->GCtD.jketim,
         "REST%B20/27IL$mV",     &RP0->GCtD.vrest,
         "VREST%B20/27IL$mV",    &RP0->GCtD.vrest,
...
*---------------------------------------------------------------------*/
   ...
   case MS_NOISE:
      inform("(S=B20/27IJ$mV,VB24/31IJ$mV,VB24IJ)",
         &RP0->GNoD.nmn, &RP0->GNoD.nsg, &RP0->GNoD.nfr, NULL);
      kwscan(&ic,
         "MEAN%B20/27IJ$mV",  &RP0->GNoD.nmn,
         "SIGMA%VB24/31IJ$mV",&RP0->GNoD.nsg,
         "FRAC%VB24IJ",       &RP0->GNoD.nfr,
         NULL);
      break;
