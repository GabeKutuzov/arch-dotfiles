...
/* $Id: d3gfsh.c 46 2011-05-20 20:14:09Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                               d3gfsh                                 *
...
static void writesel(void) {
   ...
   long llist = strlen(vlist);

   rfwi4(pgdf, (si32)(-llist));
   rfwrite(pgdf, vlist, (size_t)llist, ABORT);
...
static void addselect(int iselect, char delim) {
   ...
   ibcdwt(RK_LFTJ+RK_IORF+LSELECTOR-2, selector, iselect);
...
static long mapilist(ilst *pil, long mxi) {
   ...
   for (pcl=pil->pidl; pcl<=pcle; pcl++) {
      ...
      ibcdwt(RK_LFTJ+RK_IORF+LSELECTOR-1, celltxt, rv);
   ...
   if (ievry) {
      addfield(",EVERY ", 7);
      ibcdwt(RK_LFTJ+RK_IORF+LSELECTOR-1, celltxt, ievry);
      addfield(celltxt, RK.length+1);
      addfield(",BASE ", 6);
      celltxt[0] = '0' + ibase;
      addfield(celltxt, 1);
...
void d3gfsh (struct SDSELECT *pgd, ui32 gdm)  {
   ...
   struct AUTSCL *paut;          /* Ptr to autoscale info */
   struct CLBLK *pclb;           /* Ptr to current cell list */
   ilst *pil;                    /* Ptr to cell list being used */
   char *prevrep;                /* Name of previous repertoire */
   ilst glilst;                  /* Global cell list */
   long ldata = 0, ncells;       /* Length of data per time step */
   ui32 lsvi,lsvgl;              /* Global svitms codes */
   ui32 lsvct,lsvcn;             /* CELLTYPE and CONNTYPE suitms */
...
*  to skip writing the SEGMENT and LENGTH records, too.  */
   ...
   RP->ksv &= ~KSDSG;            /* Initially, nothing to write */
   ...
   if (RP->narms <= 0)              lsvgl &= ~(GFD1R|GFD1T);
   if (RP->nwdws <= 0)              lsvgl &= ~GFD1W;
   lsvi = lsvgl & (GFD1T|GFD1R|GFD1W|GFIA|GFEFF|GFVAL|GFOXY|GFOID|
      GFSNS|GFSID|GFUTV);
...
/* In order to simplify run-time testing, turn off svitms request bits
*  for items that do not exist.  However, items Izha-d,Lij,Cij,Dij can
*  be calculated on-the-fly even if not saved (but Dij should not be
*  output if it is never used at all).  Note also that it would
*  be possible to calculate an item solely because it is requested in
*  svitms (SBAR might be a candidate), but this is currently done only
*  for items AK and PHD, which may have to be stored in the prd data
*  structure solely for SIMDATA/REPLAY output.  (svand will indicate
*  if not saved for any conntype.)  PHD will be allocated and stored
*  only if phase calculation is being done anyway.  These tests must
*  be done after any possible change to svitms at d3grp3 time and
*  before the treee broadcast at new series.  Results are saved in
*  suitms at all levels to avoid need to repeat these tests on every
*  output cycle.  */

      lsvct = il->svitms;
      if (lsvgl & GFGLBL) lsvct |= (GFSI|GFGLBL);
      il->suitms = 0;
      /* We have to skip the celltype if Si not recorded, because
      *  there needs to be a cell list for the other variables.  */
      if (!(lsvct & GFSI)) continue;
      if (!il->nizvar)              lsvct &= ~GFABCD;
      if (!ctexists(il,SBAR))       lsvct &= ~GFSBAR;
      if (!ctexists(il,QBAR))       lsvct &= ~GFQBAR;
      if (!il->phshft)              lsvct &= ~(GFPHI|GFDIS);
      if (!ctexists(il,IZVU))       lsvct &= ~GFIFUW;
      if (!il->kautu)               lsvct &= ~GFASM;
      il->suitms = lsvct &
         (GFSI|GFGLBL|GFASM|GFIFUW|GFDIS|GFPHI|GFQBAR|GFSBAR|GFABCD);
      for (ix=il->pct1; ix; ix=ix->pct) {
         lsvcn = ix->svitms;
         if (!cnexists(ix,MIJ))     lsvcn &= ~GFMIJ;
         if (!cnexists(ix,CIJ0))    lsvcn &= ~GFCIJ0;
         if (ix->Cn.kdelay == 0)    lsvcn &= ~GFDIJ;
         if (!cnexists(ix,PPF))     lsvcn &= ~GFPPF;
         lsvcn &=
            (GFAFF|GFNUK|GFLIJ|GFCIJ|GFCIJ0|GFSJ|GFDIJ|GFMIJ|GFPPF);
         ix->suitms = lsvcn;
         il->suitms |= lsvcn;       /* If empty, kills loop below */
         } /* End CONNTYPE loop */
      lsvi |= il->suitms;           /* Ef empty, kills loop below */
      } /* End CELLTYPE loop */

   pgd->suitms = lsvi;
   if (!(lsvi &= gdm)) return;   /* Exit if nothing in this segment */
   RP->ksv |= KSDSG;             /* Indicate need to write segment */
...
/* There are items to be written, so now prepare segment header */
   ...
   /* Allocate work area--80 chars is a little long for the stack.
   *  N.B.  Must deallocate on any exits below this. */
   vlist = mallocv(MXDATA+1, "D3GFSH: vlist");
...
*---------------------------------------------------------------------*/
   ...
   convrt("(NP2,'0SIMDATA selector information for segment ',"
      "J0IH8,':')", &pgd->gdseg, NULL);
   sconvrt(vlist, "('SEGMENT',IH5,IL12)",
...
*---------------------------------------------------------------------*/
   ...
   if (lsvi & (GFD1R|GFD1T)) {
      ...
      for (pa=RP0->parm1; pa; pa=pa->pnxarm) {
         ...
         if ((lsvi & GFD1R) && (pa->njnts > 1)) {
            /* Item code J (or J and T): Output all joints */
            sconvrt(vlist, "('LEVEL 2 JOINT 1-',J1UC8,'JOINTXY')",
               &pa->njnts, NULL);
            ldata += (2*FMSSIZE)*pa->njnts; }
         else {
            /* Item code T: Output last joint only */
            sconvrt(vlist, "('LEVEL 2 JOINT ',J1UC8,'JOINTXY')",
               &pa->njnts, NULL);
            ldata += (2*FMSSIZE); }
         writesel();
         }
...
*---------------------------------------------------------------------*/
   ...
   for (il=RP->pfct; il; il=il->pnct) {
      ...
      long lcld = 0;          /* Length of cell-level data (data
                              *  sent from nodes if parallel)  */
      long lnc1;              /* Number of connections minus 1 */
      ui32 ilsv;              /* Flags for cell & connection items */
      ui32 svand;             /* Temp for GFAFF,GFNUK tests */

      if ((ilsv = il->suitms & gdm) == 0) continue;
      ir = il->pback;
...
/*---------------------------------------------------------------------*
*  11) Selectors for STATE, PHASE, IFUW, SBAR, QBAR, PHIDIST, and      *
...
*---------------------------------------------------------------------*/
      ...
      *  is set, use all cells, even if there is an explicit list.  */
      ...
      if (lsvgl & GFGLBL) {
         pil = &glilst;
         glilst.nusm1 = (il->nelt > 1);
         glilst.last = IL_REND | (il->nelt - 1);
         }
      else {                  /* Find the relevant list */
         while (pclb && pclb->pcll != il) pclb = pclb->pnclb;
         if (!pclb) d3exit(NOCLST_ERR, fmturlnm(il), 0);
         pil = pclb->pclil;
         }

      if (pil->nusm1 > 0) strcat(vlist, "S"); /* Make that "CELLS" */
      ncells = mapilist(pil, il->nelt);

      /* State, Izhu, phase, sbar, qbar, phase distribution will be
      *  written only if requested and they exist.  No error if they
      *  don't exist.  Note that lcld will get multiplied by ncells
      *  later.  Order of terms chosen to help make neater dumps.  */
      if (ilsv & GFSI) {
         addfield("STATE,", 6);
         ldata += FMSSIZE*ncells; }
      if (ilsv & GFPHI) {
         addfield("PHASE,", 6);
         ldata += ncells; }
      if (ilsv & GFIFUW) {
      ...
      if (ilsv & GFDIS) {
         addfield("PHIDIST,", 8);
         lcld += PHASE_RES*FMLSIZE; }
      if (ilsv & GFABCD) {
         /* il->prfi must exist or GFABCD bit will be off */
         struct IZHICOM *piz = (struct IZHICOM *)il->prfi;
         if (piz->izra) addfield("IZHA,", 5), lcld += FMLSIZE;
         if (piz->izrb) addfield("IZHB,", 5), lcld += FMLSIZE;
         if (piz->izrc) addfield("IZHC,", 5), lcld += FMLSIZE;
         if (piz->izrd) addfield("IZHD,", 5), lcld += FMLSIZE;
         }
...
/*---------------------------------------------------------------------*
*  12) Selectors for AK (afference) and NUK (number connections used)  *
*                                                                      *
...
*---------------------------------------------------------------------*/
      ...
      *  Logic above leaves ilsv clear if there are no conntypes.  */
      ilsv &= (GFAFF|GFNUK|GFLIJ|GFCIJ|GFCIJ0|GFSJ|GFDIJ|GFMIJ|GFPPF);
      if (!ilsv) goto NO_CONNTYPE_VARIABLES;

/* If the only variables to be written out for all connection types
*  are AK and/or NUK, and those written are written for all conntypes,
*  then a single LEVEL 4 CONNTYPE selector will do for all of them.
...
*     ctwk.gfsh.svand.  */
      ...
      if ((ilsv ^ svand) == 0) {
         ...
         if (il->nct > 1)
            sconvrt(vlist, "(T17,'S 1-',J1IH8)", &il->nct, NULL);
         else
            strcat(vlist, " 1 ");
         if (ilsv&GFAFF) strcat(vlist, "AK,"),  lcld += FMESIZE*il->nct;
         if (ilsv&GFNUK) strcat(vlist, "NUK,"), lcld += FMLSIZE*il->nct;
         /* Get rid of extra comma at the end */
...
*  each.  Add the AK and NUK variables at this level if requested.  */

      else for (ix=il->pct1; ix; ix=ix->pct) {
         ui32 ixsv = ix->suitms & gdm;
         if (!ixsv) continue;
         sconvrt(vlist, "('LEVEL 4 CONNTYPE ',J1IH8)", &ix->ict, NULL);
         if (ixsv&GFAFF) strcat(vlist, "AK,"),  lcld += FMESIZE;
         if (ixsv&GFNUK) strcat(vlist, "NUK,"), lcld += FMLSIZE;
         /* Get rid of extra comma at the end */
         vlist[strlen(vlist)-1] = '\0';
         writesel();
...
*---------------------------------------------------------------------*/
         ...
         if ((ixsv & (GFLIJ|GFCIJ|GFCIJ0|GFSJ|GFMIJ|GFDIJ|GFPPF)) == 0)
            continue;
         /* Create LEVEL 5 CONNECTION card for nc connections */
         strcpy(vlist, "LEVEL 5 CONNECTION");
         if ((lnc1 = ix->nc - 1) > 1)
            sconvrt(vlist, "(T19,'S 0-',J1IL8)", &lnc1, NULL);
         else
            strcat(vlist, " 0 ");
         /* Note order is Lij first, matching d3gfsv order */
         if (ixsv&GFLIJ)  strcat(vlist,"LIJ,"),  lcld += FMLSIZE*ix->nc;
         if (ixsv&GFCIJ)  strcat(vlist,"CIJ,"),  lcld += FMSSIZE*ix->nc;
         if (ixsv&GFCIJ0) strcat(vlist,"CIJ0,"), lcld += FMSSIZE*ix->nc;
         if (ixsv&GFSJ)   strcat(vlist,"SJ,"),   lcld += FMSSIZE*ix->nc;
         if (ixsv&GFMIJ)  strcat(vlist,"MIJ,"),  lcld += FMSSIZE*ix->nc;
         if (ixsv&GFDIJ)  strcat(vlist,"DIJ,"),  lcld += ix->nc;
         if (ixsv&GFPPF)  strcat(vlist,"PPFIJ,"),lcld += FMSSIZE*ix->nc;
         /* Get rid of extra comma at the end */
         vlist[strlen(vlist)-1] = '\0';
         writesel();
