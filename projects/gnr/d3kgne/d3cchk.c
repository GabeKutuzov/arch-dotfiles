/* (c) Copyright 2004-2006, Neurosciences Research Foundation, Inc. */
/* $Id: d3cchk.c 44 2011-03-04 22:36:14Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                              d3cchk.c                                *
*                                                                      *
         ...
         if (pion->ionflgs & (CDOP_IONEXT|CDOP_IONREV)) {
            ...
            case ION_GROUP:
               ...
               if (ICD.jr->niong >= BYTE_MAX)
                  d3exit(LIMITS_ERR, "group ion", BYTE_MAX);
               ICD.jr->niong += 1;
...
*---------------------------------------------------------------------*/
         ...
         if (lactv == CDAC_SYNAPTIC) {
            ...
            if (ic > 0) {
               ...
               if (!ix) d3exit(CONDCT_ERR,   /* A development error */
                  fmturlnm(il), (long)pcnd->iact);
...
*---------------------------------------------------------------------*/
         ...
         *  generate one from the master seed for the celltype.  */
         ...
         if (il->ncnd >= BYTE_MAX)
            d3exit(LIMITS_ERR, "conductance", BYTE_MAX);
...
*---------------------------------------------------------------------*/
         ...
         if (pcnd->gDCY.omega != 0) il->ctf |= CTFgD;
         if (lcond == CDTP_ALPHA || lcond == CDTP_DOUBLE) {
            ...
            if (pcnd->gDCY.omega)
               ...
               if (ngcyc > UI16_MAX) {
                  char cct[10];
                  ibcdwt(RK_LFTJ|RK_IORF|8, cct, ngcyc);
   ...
   for (ir=RP->tree; ir; ir=ir->pnrp) {
      ...
      for (il=ir->play1; il; il=il->play) {
         ...
         if (ICD.tionc > BYTE_MAX)
            d3exit(LIMITS_ERR, "ion", BYTE_MAX);
         il->nionc = ICD.tionc;
         }
      }
