...
/* $Id: d3fchk.c 46 2011-05-20 20:14:09Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                               d3fchk                                 *
*                                                                      *
...
struct SFALLHDRS {
   ...
   struct {    /* Temps for checking retrieved D1 info */
      long tnepr;
      short tnd1r;
      short tnbpe;
      } td1;
   ...
   struct {    /* Temps for checking retrieved cn info */
      ui32  tnkgen,tnradius;
      long  tnnc,tnsngx,tnsngy,tnsnel,tnsnelt;
      long  tnloff,tnlsg,tnnrx,tnnry,tnnsa,tnnsax;
      long  tnnux,tnnuy,tnstride,tnxoff,tnyoff;
      short tcnoff[CNNDV],txnoff[XNNDV];
      ui16  tnlc,tnlxn;
      short tnnbc,tncbc;
      short tnnhx,tnnhy,tnnvx,tnnvy;
      } tcn;
      ...
      if (GotMatch) {         /* Key name was matched */
         ...
         *  CNS, the test here can be revised to allow that case.  */
         if (((rdhkey.type & SFTPM) != ktyp) ||
            ...
            d3exit(RESTYP_ERR, pkb->key, pkb->type);
...
*  reading each item individually saves allocating a temp array.  */
         ...
         for (idim=0; idim<idime; idim++) {
            ...
            case UTYPE: {
               register unsigned long tul;
               if (rdhkey.len == FMSSIZE)
                  tul = (unsigned long)bemtoi2(cdat);
               else if (rdhkey.len == FMLSIZE)
                  tul = bemtoi4(cdat);
               else
                  d3exit(RESTYP_ERR, pkb->key, pkb->type);
               /* Need braces: bemfmi[24] can be macros */
               if (pkb->len == FMSSIZE) {
                  bemfmi2(pdata, (unsigned short)tul); }
               else if (pkb->len == FMLSIZE) {
                  bemfmi4(pdata, tul); }
               else
                  d3exit(RESTYP_ERR, pkb->key, pkb->type);
...
/* Check that all required items have been found in the data */
   ...
   for (pkb=pkbf; pkb<pkbl; pkb=pnkb) {
      if ((pkb->type & (SFGOT|SFREQ)) == SFREQ)
         d3exit(RESITM_ERR, pkb->key, pkb->type);
      litem = sizeof(SFHKey) + (int)pkb->len*(int)pkb->dim;
      pnkb = (SFHKey *)((char *)pkb + litem);
...
void d3fchk(void) {
...
#ifdef D1
   D1TYPE            *pd1;    /* Ptr to D1BLK */
   struct VBDEF      *ivb;    /* Ptr to value block */
   struct D1FHDR      fhd;    /* DIGINPUT file header */
   struct D1FBHD      bhd;    /* DIGINPUT block header */
#endif
...
#ifdef D1
   int d1rc;                  /* One of the above return codes */
   int idh;                   /* D1 array header counter */
   int idhe;                  /* Ending value of idh */
   int id1;                   /* D1 array number for emsg */
#endif
   ...
   static char *hwrntxt[] =   /* Warnings by header class */
      { "cell data", "connection type", "gconn type",
        "modulation type" };
#ifdef D1
   static char *D1ErrType[] = /* D1 input file errors */
      { "Header", "Version" };
#define D1_HEAD_ERR  0        /* Header mismatch */
#define D1_VERS_ERR  1        /* Version mismatch */
#endif
...
*  values set here remain in place for use by d3save.  */
   ...
   bemfmi4(ursfhdr2.sfhdblk.blklen, (long)sizeof(struct SFHDR2));
#ifdef D1
   bemfmi4(ursfd1hdr.sfd1blk.blklen, (long)sizeof(struct SFD1HDR));
#endif
   bemfmi4(ursfrephdr.sfrepblk.blklen, (long)sizeof(struct SFREPHDR));
   bemfmi4(ursfcthdr.sfctblk.blklen, (long)sizeof(struct SFCTHDR));
   bemfmi4(ursfcnhdr.sfcnblk.blklen, (long)sizeof(struct SFCNHDR));
   bemfmi4(ursfcghdr.sfcgblk.blklen, (long)sizeof(struct SFCGHDR));
   bemfmi4(ursfcmhdr.sfcmblk.blklen, (long)sizeof(struct SFCMHDR));
...
/* Loop over SAVENET files to be restored */
   ...
   for (psvf=d3file[SAVREP_IN],ifile=1; psvf; psvf=psvf->nf,ifile++) {
      ...
      if (memcmp(h->sfhdr.filetype, SF_IDENT, SFVERS_LEN-4) ||
         ibcdin(RK_IORF+SFVERS_LEN-1, h->sfhdr.filetype) <
            SF_VERS) {
...
#ifdef D1
/*---------------------------------------------------------------------*
*             Read Darwin 1 headers and try to match them              *
*  Unlike the case with normal CNS repertoires, saved D1 repertoires   *
*  are only restored into the first matching repertoire in the current *
*  run.  This is because lack of names makes false matches too easy.   *
*---------------------------------------------------------------------*/

      idhe = (int)bemtoi2(h->sfhdr2.sfhnd1b.d);
      for (idh=1; idh<=idhe; idh++) {

         readsfhd(psvf, &h->d1hdr, ursfd1hdr);
         h->td1.tnepr = bemtoi4(h->d1hdr.sfd1nepr.d);
         h->td1.tnd1r = bemtoi2(h->d1hdr.sfd1nd1r.d);
         h->td1.tnbpe = bemtoi2(h->d1hdr.sfd1nbpe.d);

         /* Check header against current D1 arrays */
         for (pd1=RP->pd1b1; pd1; pd1=pd1->pnd1b) {
            /* Ignore if override or already matched */
            if (pd1->kd1o & KD1_OVER || pd1->kd1f & KD1_MATR)
               continue;
            /* Ignore if other file requested by user */
            if (pd1->rfile != 0 && pd1->rfile != ifile) continue;

            /* Match fields that must agree */
            kbaditem = 0;
            if (pd1->nepr != h->td1.tnepr) kbaditem |= 1;
            if (pd1->nd1r != h->td1.tnd1r) kbaditem |= 2;
            if (pd1->nbpe != h->td1.tnbpe) kbaditem |= 4;
            /* Not a match--go on to next D1 array.
            *  If D1 arrays ever have names, need error message
            *  here if pd1->rfile is equal to current fileno.  */
            if (kbaditem) continue;

            /* Got a match--set to use this input for restore */
            pd1->rfile = ifile;  /* Doesn't hurt if already set */
            pd1->kd1f |= KD1_MATR;
            if (SFHGot(d1hdr, sfd1seed))
               pd1->d1seed = bemtoi4(h->d1hdr.sfd1seed.d);
            pd1->D1offsi = bemtoi4(h->d1hdr.offd1s.d);
            pd1->D1offrd = bemtoi4(h->d1hdr.offd1r.d);
            break;               /* On to next header */
            } /* End D1 block loop */
         } /* End D1 hdr loop */
#endif

/*---------------------------------------------------------------------*
*       Read repertoire and layer headers and try to match them        *
...
for (irh=1; irh<=irhe; irh++) {
   ...
   h->trp.trngx = bemtoi4(h->rephdr.sfrngx.d);
   h->trp.trngy = bemtoi4(h->rephdr.sfrngy.d);
...
/* We got a match--save needed fields in celltype */
         ...
         il->ctwk.phs1.offsi =    bemtoi4(h->cthdr.offsi.d);
         il->ctwk.phs1.offprd =   bemtoi4(h->cthdr.offprd.d);
         il->ctwk.phs1.rflel =    bemtoi4(h->cthdr.sfctlel.d);
         il->ctwk.phs1.rfphshft = bemtoi2(h->cthdr.sfctphshft.d);
         il->ctwk.phs1.rfdelay =  bemtoi2(h->cthdr.sfctsdly1.d);
         /* Copy updated autoscale multiplier if found and exists
         *  and "override restore" was not specified.  */
         if (SFHGot(cthdr, sfctasm) && !(il->Ct.ctopt & OPTOR) &&
            il->kautu) RP->psclmul[il->oautsc] =
               bemtoi4(h->cthdr.sfctasm.d);

         /* Use updated seeds from file header */
         if (SFHGot(cthdr, sfctnsd))
            il->nsd = bemtoi4(h->cthdr.sfctnsd.d);
...
*---------------------------------------------------------------------*/
      ...
      for (ixh=1; ixh<=ixhe; ixh++) {
         ...
         h->tcn.tnsngx =   bemtoi4(h->cnhdr.sfcnsrcngx.d);
         h->tcn.tnsngy =   bemtoi4(h->cnhdr.sfcnsrcngy.d);
         ...
         h->tcn.tnnrx =    bemtoi4(h->cnhdr.sfcnnrx.d);
         h->tcn.tnnry =    bemtoi4(h->cnhdr.sfcnnry.d);
         ...
         h->tcn.tnnvx =    bemtoi2(h->cnhdr.sfcnnvx.d);
         h->tcn.tnnvy =    bemtoi2(h->cnhdr.sfcnnvy.d);
...
*  Match data fields that must agree.  */
            ...
            *  same as current nuklen inasmuch as nc mustbe same.
            ...
            *  generating code--only test variables that are in use */
            ...
            if (ix->kgen & (KGNJN|KGNND|KGNOG|KGNYO) &&
            ...
            *  this way.  */
            if (ix->kgen & (KGNFU|KGNTP|KGNBV) || (il->nelt > 1 &&
                  ix->kgen & (KGNFU|KGNTP|KGNJN|KGNXO|KGNYO|KGNBV))) {
               if (h->tcn.tnnux  != ix->nux)    kbaditem |= 0x00010000;
               if (h->tcn.tnnuy  != ix->nuy)    kbaditem |= 0x00020000;
               }
            if (ix->kgen & (KGNAJ|KGNBL) &&
                h->tcn.tnstride != ix->stride)  kbaditem |= 0x00040000;
            if (ix->kgen & (KGNFU|KGNTP|KGNJN|KGNXO|KGNYO|KGNBV)) {
               if (h->tcn.tnxoff != ix->xoff)   kbaditem |= 0x00080000;
               if (h->tcn.tnyoff != ix->yoff)   kbaditem |= 0x00100000;
               }
            if (ix->kgen & (KGNDG|KGNAN)) {
               if (h->tcn.tnnhx  != ix->nhx)    kbaditem |= 0x00200000;
               if (h->tcn.tnnhy  != ix->nhy)    kbaditem |= 0x00400000;
               }
            if (ix->kgen & (KGNDG)) {
               if (h->tcn.tnnvx  != ix->nvx)    kbaditem |= 0x00800000;
               if (h->tcn.tnnvy  != ix->nvy)    kbaditem |= 0x01000000;
               }
...
/* We got a match--save needed fields in conntype block */
            ...
            *  prevent future matches of other implicit rct's.  */
            ...
            ix->cnwk.phs1.offprd = bemtoi4(h->cnhdr.offprd.d);
            ix->cnwk.phs1.rflel =  bemtoi4(h->cthdr.sfctlel.d);
...
*  from files, thereby activating d1genr or d3genr to generate them.   *
...
#ifdef D1
/* Check for unmatched D1 arrays and give warnings */
   ...
   for (pd1=RP->pd1b1,id1=1; pd1; pd1=pd1->pnd1b,id1++) {
      ...
      if (Restoring) {
         if (pd1->rfile) {
            convrt("(P1A1,'***D1 ARRAY ',J1I4,"
               "'NOT MATCHED W/RESTORE FILE ',J0IH4)",
               &Emcc, &id1, &pd1->rfile, NULL);
            RK.iexit |= FILE_ERR; }
         else
            convrt("(P1A1,'-->WARNING: D1 array ',J1I4,"
               "'not matched w/restore file, will generate.')",
               &Emcc,&id1,NULL);
         Emcc = ' ';
         } /* End if Restoring */
      pd1->kd1o |= KD1_OVER;
...
#ifdef D1
/*---------------------------------------------------------------------*
*     (2) Open and check DIGINPUT files                                *
*---------------------------------------------------------------------*/

/*    Notes:  d3grp1 will have already checked that no D1BLK refers
*  to a file number greater than the number loaded, including zero,
*  and d3tchk will have deleted all unused D1BLKs.  This routine
*  deletes unused input RFdefs.  File descriptors are not saved in
*  D1BLKs.
*     The field pd1->odin in D1BLK gives offset to the input data
*  record, which may contain an index followed by one (if indexed)
*  or more group, string pairs.  It is an error if user said no
*  group numbers, but the input has them.
*     The RFdef->userdata field is used for two purposes:
*     (1) The number of D1BLKs that expect to read from this file
*  is counted to be sure it matches the number of blocks for which
*  the file has data.  Otherwise, reading would get out of step.
*     (2) Upon exit from d3fchk, userdata contains the seek offset
*  to be applied when the file is reopened in phase 2 to skip over
*  the headers.
*     KLUDGE ALERT:  The RFdef->lrecl field is used by this program
*  to store the length of the record that must be read on each cycle.
*  This length includes the group number if the file is version 2.
*     RESTRICTIONS:  All the data in each file have to be used.
*  The data records in the input files have to be in the same order
*  as the D1BLKs that use them--there are no names to match up.  */

   /* Omit all checking if no D1 blocks are active */
   if (RP0->cumnd1) {
      struct RFdef *pdf,**pldf; /* Ptr to RFdefs */
      byte *psj = RP->pd1in;  /* Ptr to input buffer */
      int jf;                 /* File counter */
      int kndx =              /* Indexing switch */
         (RP->pd1b1->kd1o & KD1_INDX) ? DHM_INDX : DHM_NORM;

      /* Clear the record length in lrecl and the usage count in
      *  userdata for all DIGINPUT files */
      for (pdf=d3file[DIGINPUT]; pdf; pdf=pdf->nf)
         pdf->userdata = pdf->lrecl = 0;

      /* Loop over D1 blocks, locate input RFdefs,
      *  and increment usage counts by file */
      for (pd1=RP->pd1b1; pd1; pd1=pd1->pnd1b) {
         for (pdf=d3file[DIGINPUT], jf=pd1->jfile-1;
            jf; pdf=pdf->nf, --jf) ;
         pdf->userdata++;
         }

      /* Loop over files and check headers */
      for (pldf=d3file+DIGINPUT, jf=1; *pldf; jf++) {
         pdf = *pldf;

         /* If file is not used, delete its RFdef entry */
         if (pdf->userdata == 0) {
            *pldf = pdf->nf;
            free(pdf->fname);
            free(pdf);
            continue;
            } /* End deleting input file */

         rfopen(pdf,NULL,SAME,SAME,SAME,SAME,SAME,SAME,SAME,
            SAME,SAME,SAME,ABORT);
         rfread(pdf,&fhd,sizeof(struct D1FHDR),ABORT);

         if (memcmp(fhd.dhtype,"D1IN",4) ||
            (bemtoi2(fhd.dhmode) != kndx) ||
            (bemtoi4(fhd.dhnd1b) != pdf->userdata))
            { d1rc = D1_HEAD_ERR; goto DigInputFileError; }

         /* File header is OK, now check block headers */
         for (pd1=RP->pd1b1; pd1; pd1=pd1->pnd1b) {
            if (pd1->jfile == jf) {
               register int lidr;   /* Length of input data record */

               rfread(pdf, &bhd, sizeof(struct D1FBHD), ABORT);

               if (bemtoi2(bhd.dhnd1r) != pd1->nd1r ||
                  bemtoi2(bhd.dhnbpe) != pd1->nbpe)
                  { d1rc = D1_HEAD_ERR; goto DigInputFileError; }

               if ((pd1->kd1o & KD1_VER1) && (bemtoi2(fhd.dhvers) > 1))
                  { d1rc = D1_VERS_ERR; goto DigInputFileError; }

               /* Set D1BLK to fetch input from this file.  Note that
               *  id1gn length is calculated from fhd version, not from
               *  D1BLK vers, as file may be V1 while D1BLK is V2. */
               pd1->li1r = pd1->nbyt;
               if (bemftoi2(fhd.dhvers) > 1) pd1->li1r += sizeof(short);
               else                          pd1->kd1o |= KD1_VER1;
               lidr = kndx ?
                  FMISIZE + pd1->li1r : pd1->nd1r * pd1->li1r;
               pd1->pdin = psj, psj += lidr;
               pdf->lrecl += lidr;
               } /* End if block uses this file */
            } /* End loop over blocks */

         /* Close file, then save offset for restarting at data */
         rfclose(pdf, SAME, SAME, ABORT);
         pdf->userdata = sizeof(struct D1FHDR) +
            bemtoi4(fhd.dhnd1b) * sizeof(struct D1FBHD);
         goto ProcessNextFile;

DigInputFileError:             /* Error exit */
         cryout(CROFE,"0***",RK_LN2+4,D1ErrType[d1rc],RK_CCL,
            " ERROR ON D1 INPUT FILE ",RK_CCL,
            pdf->fname,RK_CCL,NULL);
ProcessNextFile:              /* Normal loop continuation */
         pldf = &pdf->nf;
         } /* End checking DIGINPUT files */

/* Now that all the pdin's are assigned, it is possible to initialize
*  any D1 value schemes so they can find the group numbers quickly. */

      for (ivb=RP0->pvblke; ivb; ivb=ivb->pnxvt)
         if (ivb->vbkid == VBK_D1) {
            struct D1BLK *pd1 = ivb->u1.vd1.pvsd1;
            ivb->u1.vd1.ovgrp = pd1->odin +
               ivb->u1.vd1.repnm1*pd1->li1r +
               ((pd1->kd1o & KD1_INDX) ? FMISIZE : 0);
            } /* End D1 value setup */

      } /* End if Darwin 1 input active */
#endif
