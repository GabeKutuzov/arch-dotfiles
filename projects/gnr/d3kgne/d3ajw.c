...
/* $Id: d3ajw.c 51 2012-05-24 20:53:36Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                               d3ajw.c                                *
*                                                                      *
...
void g1arb(char *type, struct EFFARB *pea) {
   ...
   *  for EXCIT, INHIB.  */
#define NEAPS 12              /* Number of effarb parameters */
...
#define ATTNUM 3              /* Number of arm attachment types */
   ...
   float jmn0, jmx0, scla0, ja00, jl00;
   ...
   *(AJW.pla = &pa->pnxarm) = NULL;
   ...
   if (RP->narms >= BYTE_MAX)
      d3exit(LIMITS_ERR, "arm", BYTE_MAX);
   pa->armno = ++RP->narms;
   ...
   if (!(nj = pa->njnts)) {
      ...
      for (j=1; j<=nje; pj++,pea++,j++) {
         ...
         *pea = tefa;    /* Copy arbor defaults */
         ...
         if (j>nj)  /* UNIVERSAL JOINT */  {
            /* Initialize universal joint block */
            pj->jl            =  0.0;
            ...
            /* Initialize regular joint block */
            pj->jl            = jl00;
            jl00 = max(jl00-1.0,4.0);
            pj->u.jnt.ja      = ja00;
            pj->u.jnt.jmn     = jmn0;
            pj->u.jnt.jmx     = jmx0;
            pj->u.jnt.tla     = -1.0;
      ...
      if (pa->jatt&ARMGE) {   /* GRIP JOINT */
         /* Initialize grip joint block */
...
void g1grip(void) {
   ...
   inform("(SW1=VIL,B8Il)",&pgj->u.grip.grvi,
      &pgj->u.grip.grvt,NULL);
   kwscan(&ic,
      "VI%VIL", &pgj->u.grip.grvi,
      "VT%B8IL", &pgj->u.grip.grvt,
...
void g1jnt(void) {
   ...
   kwscan(&ic,
         ...
         "TLA%V>F",           &pj->u.jnt.tla,
         "TLL%V>F",           &pj->u.jnt.tll,
         "TZ%IL",             &pj->u.jnt.tzi,
         "SCLA%P3F$-/mV",     &pj->scla,
         "%%J" KWJ_GARB "J1", pea,
         NULL);

   /* Check that touch length not > overall length */
   if (pj->u.jnt.tla > pj->jl)
...
void g1ujnt(void) {
   ...
   kwscan(&ic,
         ...
         "VI%VIL",            &puj->u.ujnt.ujvi,
         "VT%B8IL",           &puj->u.ujnt.ujvt,
...
void g1wdw(void) {
   ...
   if (RP->nwdws >= BYTE_MAX)
      d3exit(LIMITS_ERR, "window", BYTE_MAX);
   pw->wdwno = RP->nwdws++;
...
void g1eff(void) {
   ...
   *  unique to EFFECTOR can be added here as needed */
   ic = 0;
   kwscan(&ic,
      "NCHGS%V>IH",        &pea->nchgs,
      "EOPT%P2KCFC",       &pea->keop,
...
void d3fix1(void) {
   ...
   RP->nsx = 1L<<RP->kx;
   RP->nsy = 1L<<RP->ky;
   RP->xmask = (RP->nsx - 1L) << RP->ky1;
   RP->ymask = RP->nsy - 1L;
   RP->xymaskc = RP->nsy*(1L - 2*RP->nsx);
   RP->xymask = ~RP->xymaskc;
