/* (c) Copyright 1986-2011, Neurosciences Research Foundation, Inc. */
/* $Id: d3gfhd.c 51 2012-05-24 20:53:36Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                               d3gfhd                                 *
...
#define NVARS     31       /* Number of variables in data structure */
...
*     type     Code for type of name, defined in simdata.h             *
...
static void writevar(char *name, int type, int level, int scale,
   ...
   sconvrt(vardes, "(O,A" qLVN ",4*I4IL8)",
...
*  to activate separately later.)  */
   ...
   writevar("CAMID",  FTYPE, 1, 0, SHSIZE, 2);
   writevar("CAMINFO",UTYPE, 1, 0, BSIZE, NUTVINFO);

/* Variable 7: Value */

   writevar("VALUE",  FTYPE, 1, FBsv, SHSIZE, 0);
...
/* Variable 9: Sense ID (isn,ign) data */
   ...
   writevar("SNSID",  UTYPE, 1, 0, SHSIZE, 2);

/* Variable 10: Joint x,y coords */

   writevar("JOINT",  FTYPE, 2, 3, SHSIZE, 2);
...
/* Variable 12: Autoscale multiplier (one per celltype) */
   ...
   writevar("ASM",    UTYPE, 2, FBsc, I32SIZE, 0);

/* Variable 13: s(i) for cells */

   ibs = FBsi + RP->bsdc;
   jbs = FBwk + RP->bsdc;
   writevar("STATE",  FTYPE, 3, ibs, SHSIZE, 0);

/* Variable 14: sbar (time-average s) for cells */

   writevar("SBAR",   FTYPE, 3, ibs, SHSIZE, 0);

/* Variable 15: qbar (slower time-average s) for cells */

   writevar("QBAR",   FTYPE, 3, ibs, SHSIZE, 0);

/* Variable 16: phase for cells */

   writevar("PHASE",  UTYPE, 3, 0, BSIZE, 0);

/* Variable 17: phase distribution for cells */

   writevar("PHIDIST",FTYPE, 3, jbs, LSIZE, PHASE_RES);

/* Variable 18: Integrate & Fire IZHI 'u', BREG 'w'  */

   writevar("IFUW",   FTYPE, 3, FBIu, I32SIZE, 0);

/* Variables 19-22: Izhikevich 'a','b','c','d' parameters */

   writevar("IZHA",   FTYPE, 3, FBIab, SHSIZE, 0);
   writevar("IZHB",   FTYPE, 3, FBIab, SHSIZE, 0);
   writevar("IZHC",   FTYPE, 3, jbs, SHSIZE, 0);
   writevar("IZHD",   FTYPE, 3, jbs, SHSIZE, 0);

/* Variable 23: afference for a connection type */

   writevar("AK",     ETYPE, 4, 0, ESIZE, 0);

/* Variable 24: number of connections used */

   writevar("NUK",    UTYPE, 4, 0, LSIZE, 0);

/* Variable 25: Lij for connection--call it signed
*  so skipped conns will register as '-1'  */

   writevar("LIJ",    FTYPE, 5, 0, LSIZE, 0);

/* Variable 26: Cij for connection */

   writevar("CIJ",    FTYPE, 5,15, SHSIZE, 0);

/* Variable 27: Cij0 for connection */

   writevar("CIJ0",   FTYPE, 5,15, SHSIZE, 0);

/* Variable 28: Sj for connection */

   writevar("SJ",     FTYPE, 5, ibs, SHSIZE, 0);

/* Variable 29: Mij for connection */

   writevar("MIJ",    FTYPE, 5, FBsi+RP->bsdc, SHSIZE, 0);

/* Variable 30: Dij for connection */

   writevar("DIJ",    UTYPE, 5, 0, BSIZE, 0);

/* Variable 31: PPFij for connection */

   writevar("PPFIJ",  FTYPE, 5,12, SHSIZE, 0);
