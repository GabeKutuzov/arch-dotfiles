...
/* $Id: d3vitp.c 42 2011-01-03 21:37:10Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                               d3vitp                                 *
*                                                                      *
...
*     important to remember that these may be 1, 2, or 3 bytes wide.   *
...
/* Clear the virtual cells for this touchpad */
   ...
   memset((char *)pv0, 0, pitp->vc.ncells);

/* A few handy constants */

   ipix = max(pitp->vc.vcxc.tvmode, 1);
   ipiy = ipix*pitp->tvx;
   npx =  pitp->vc.nvx;
   npy =  pitp->vc.nvy;
   tvx1 = pitp->tvx - 1;
   ...
   for (irow=0; irow<npy; ++irow) {
      ...
      for (icol=0; icol<npx; ++icol) {
         ...
         *  locate start of the Y row in the image.  */
         ...
         iybig = (int)ybig; if (iybig > tvy1) iybig = tvy1;
         pym = pim + ipiy*iylit;
