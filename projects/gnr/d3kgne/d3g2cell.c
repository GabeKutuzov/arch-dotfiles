/* (c) Copyright 1996-2011, Neurosciences Research Foundation, Inc. */
/* $Id: d3g2cell.c 50 2012-05-17 19:36:30Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                              d3g2cell                                *
*                                                                      *
...
/*---------------------------------------------------------------------*
*                               getffn                                 *
*  This little routine interprets the PHIFN option on the AMPLIF,      *
*  PARAMS, and THRESHOLDS cards.                                       *
*---------------------------------------------------------------------*/

/* Followig option names must be in same order as phifn codes in
*  the CNDFLT structure.  Global because used in d3echo().  */
char *phifnnm[] = { "DEFAULT", "1", "ABSCIJ" };

int getffn(void) {

   int mrc = match(RK_EQCHK, RK_MREQF, ~RK_COMMA, 0, phifnnm, 3);
   return (mrc > 0) ? mrc - 1 : 0;
...
int getshape(void) {
   ...
   *  in misc.h  */
   static char *sishp[] = { "CIRCLE", "SQUARE", "3BAR", "2BAR",
      "DOWN-TRIANGLE", "LEFT-TRIANGLE",
      "UP-TRIANGLE", "RIGHT-TRIANGLE" };

   return match(RK_EQCHK, RK_MREQF, ~RK_COMMA, 0, sishp,
      sizeof(sishp)/sizeof(char *));
...
*  CELLTYPE, CELLTYPE, and CHANGE control cards.  Note that the        *
*  intention is that color sources override corresponding options      *
*  in kctp, but SOLID/OPEN just updates kctp, i.e. last entered wins.  *
*---------------------------------------------------------------------*/
...
void getsicol(struct CTDFLT *pctd) {
   ...
   *  SiColors enum in celltype.h, followed by other options */
#define Sic_Solid  7
#define Sic_Open   8
   ...
   do {
      icol = match(RK_NMERR, RK_MSCAN,
         ~(RK_COMMA|RK_INPARENS|RK_RTPAREN), 0,
         sicol, sizeof sicol/sizeof(char *));
      if (icol == 0) {
         /* Unmatched name will be taken as a color name */
         scanagn(); eqscan(pctd->sicolor, "VA" qCMC, 0);
         pctd->ksipce &= ~SIC_MASK;
         pctd->ksipce |= (byte)(SIC_Fixed|SIC_COLE); }
      else if ((enum SiColors)icol <= SIC_Izhu) {
         pctd->ksipce &= ~SIC_MASK;
         pctd->ksipce |= (byte)icol; }
      else if (icol == Sic_Solid) {
         pctd->kctp |= KRPCO;
         pctd->ksipce |= SIC_SOPE; }
      else if (icol == Sic_Open) {
         pctd->kctp &= ~KRPCO;
         pctd->ksipce |= SIC_SOPE; }
      /* Negative icol (error) will drop through to while test,
      *  thereby skipping to end of parens before returning */
...
/* Read data from REGION or REPERTOIRE card */
   ...
   inform("(S=VIH,VIL)", &ir->nqx, &ir->nqy, NULL);
   ir->ngrp = ir->ngx*ir->ngy;
   kwscan(&ic,
      ...
      "VXO%F.0",              &ir->vxo,
      "VYO%F.0",              &ir->vyo,
      "RPTIMER%N" KWS_TIMNW,  &ir->Rp.jrptim,
...
*  pt, nt, st, etc. to a new CELLTYPE-level THRESHOLDS card.  */
   ...
   inform("(SW1A" qLSN "VIL=" qNCTT "B20/27IL$mV)",
   ...
   *  must match KRM_ defs in celltype.h  */
   while (kwret = kwscan(&ic,    /* Assignment intended */
         ...
         "VREST%X",
         "REST%X",                                 /* For compat */
         "SPIKE%VB20/27IJ$mV",   &il->Ct.sspike,   /* ic bit 6 */
         "THAMP%B20/27IJ$mV",    &il->Ct.stanh,    /* ic bit 7 */
         "PT%B20/27IL$mV",       &il->Ct.pt,
         "ST%B20/27IL$mV",       &il->Ct.st,
         "NT%B20/27IL$mV",       &il->Ct.nt,
         "HT%B20/27IL$mV",       &il->Ct.ht,
         ...
         "BITS%V>1<=16IH",       &RP0->LCnD.nbc,
         "SVITMS%KJ" oksimct,    &il->svitms,
         "RFILE%VIH",            &phm->fno,
         "KCTP%KL" okctkrp,      &il->Ct.kctp,
         "KETIMER%N" KWS_TIMNW,  &il->Ct.jketim,
         "KP%KL" okctkrp,        &il->Ct.kctp,
         NULL))
      switch (kwret) {
case 1:  /* RF (response function) option */
         ...
         break;
case 2:  /* SHAPE option */
         il->Ct.ksiple = (byte)getshape();
         break;
case 3:  /* COLOR option */
         getsicol(&il->Ct);
         break;
case 4:  /* VREST option */
case 5:  /* REST (compat) option */
   ...
   *  use to set default rspmeth (left byte of ic either endian).  */
   memcpy((char *)&il->ctwk.tree.krmop, (char *)&ic, 1);
...
/* Error checking for CELLTYPE card */
   ...
   *  Until we go to 64-bit code, an absolute restriction is set by
   *  cell list storage scheme, which requires two high-order bits of
   *  a long for encoding ranges and strides.  */
   il->nelt = il->nel*ir->ngrp;
   if (il->nelt >= 1073741824L)
      cryout(RK_E1, "0***NEL*NGRP EXCEEDS 1073741824.", RK_LN2, NULL);
   ...
   *  overridden by later AEIF or IZHIKEVICH card).  */
   ...
   /* Kill all stats for duration of run if RP_NOSTAT is
   *  set in runflags. */
   if (RP->CP.runflags & RP_NOSTAT) {
      il->Ct.kctp &= ~(KRPXR|KRPHR|KRPYM|KRPZC|KRPCL|KRPGP|KRPMG|KRPFD);
      il->Ct.kctp |= CTPNS; }
...
/* Update quantities derived from quantities on CELLTYPE card */
   ...
   if (ir->nlyr >= BYTE_MAX)
      d3exit(LIMITS_ERR, "layers", BYTE_MAX);
   ir->nlyr++;
   ir->ncpg += il->nel;
   } /* End g2cell() */
...
void g2amplif(struct CNDFLT *pdd) {
   ...
   hpcfn = 0;
   rule[0] = '\0';
   inform("(S=A" qDML "B28IL,J" KWJ_THR "CR,J" KWJ_THR "X,B8IH,J"
      KWJ_THR "CR,J" KWJ_THR "X,B16IL)",
      rule, &pdd->delta, &pdd->mti, &pdd->mtj, &pdd->mtv,
...
void g2autsc(struct AUTSCL *paut) {
   ...
   inform("(S=VIJ,V>0<=1B15UH)", &paut->ngtht, &paut->adamp, NULL);
   kwscan(&ic,
      "NGTHT%VIJ",            &paut->ngtht,
      "ASMXD%V>0<1B24IJ",     &paut->asmxd,
      "ASMXIMM%V>0<10B24IJ",  &paut->asmximm,
      "ASMNIMM%V>0<1B24IJ",   &paut->asmnimm,
      "ADAMP%V>0<1B15UH",     &paut->adamp,
      "OPT%KH" okautsc,       &paut->kaut,
      NULL);
   if ((paut->kaut & (KAUT_ET|KAUT_IMM)) == (KAUT_ET|KAUT_IMM))
      ermark(RK_EXCLERR);
...
*  pushing back any keywords for processing below.  */
   ...
   inform("(S=V>=0<=1B30IL)", &pcd->omega1, NULL);
   scan(lea, 0);
   if (RK.scancode & RK_ENDCARD) return;
   else if ((RK.scancode & ~(RK_BLANK|RK_COMMA|RK_INPARENS)) == 0) {
      scanagn(); getdecay(&pcd->CDCY);
      inform("(S=V<=1B15UH)", &pcd->sdamp, NULL); }
   else if (RK.scancode & RK_EQUALS) {
      scanagn(); }
   else {
      ermark(RK_PUNCERR);
      skip2end();
...
/* Read keyword parameters */
   ...
   kwscan(&ic, /* Assignment intended */
      "OMEGA1%V>=0<=1B30IL",  &pcd->omega1,
      "OMEGA2%N" KWS_DECAY,   &pcd->CDCY,
      "DECAY%N" KWS_DECAY,    &pcd->CDCY,
      "SIET%B20/27IL$mV",     &pcd->siet,
      ...
      "GAMMA%VB24IL",         &pdd->gamma,
      "EQUIL%B16IL",          &pdd->target,
      NULL);
...
case DLY_NORM:  /* Process NORMAL delay.
      ...
      *  be generated.  */
      inform("(S,VB20IL,VB24IL)", &pdd->dmn, &pdd->dsg, NULL);
      /* Calculate max delay as mean + 3*sigma.  Allow for
      *  round-off constant added in d3genr.  */
      tlong = (pdd->dmn + 3*(pdd->dsg>>4) + (S20>>1)) >> 20;
      pdd->mxcdelay = min(tlong, 255);

      } /* End kind of delay switch */

   /* Check for seed as keyword parameter */
   kwscan(&ic, "SEED%RVIL", RP0->notmaster, &pdd->dseed, NULL);
   } /* End g2delay() */
...
*                        Process 'NOISE' card                          *
*                                                                      *
*  Synopsis:  void g2noise(struct CELLTYPE *il)                        *
*                                                                      *
*  Rev, 04/07/07, GNR - Remove obsolete TYPE=M, use NMOD card          *
*---------------------------------------------------------------------*/

void g2noise(struct CELLTYPE *il) {
...
*  TYPE=S sets nfr <-- nsg, nsg <-- 0.  */
   ...
   lea1[0] = lea2[0] = '\0';
   inform("(SW1=B20/27IJ$mV,A" qDML ",A" qDML ",V>IL)",
      &il->No.nmn, lea1, lea2, &num, NULL);
   if (lea1[0] != '\0')
      sinform(lea1, "(VB24/31IJ" qDML "$mV)", &il->No.nsg, NULL);
   if (lea2[0] != '\0') {
      wbcdin(lea2, &msd, RK_CTST+RK_QPOS+RK_IORF+RK_NLONG+14);
      if (msd > 1) {
         il->nsd = msd;
         if (num != -1) ermark(RK_TOOMANY);
         }
      else {
         wbcdin(lea2, &il->No.nfr, 24*RK_BSCL+RK_CTST+RK_QPOS+
            RK_IORF+RK_NI32+14);
         if (num != -1) il->nsd = num;
         }
      }
   kwscan(&ic,
      "MEAN%B20/27IJ$mV",     &il->No.nmn,
      "SIGMA%VB24/31IJ$mV",   &il->No.nsg,
      "FRAC%VB24IJ",          &il->No.nfr,
      "SEED%V>IL",            &il->nsd,
      "TYPE%KIS",             &kode,
      NULL);
   if (kode & 1) {               /* Got TYPE=S */
      /* If seed was entered positionally, it was picked up above */
      if (bittst((byte *)&ic, 2)) ermark(RK_EXCLERR);
      if (lea1[0] != '\0') wbcdin(lea1, &il->No.nfr,
         24*RK_BSCL+RK_CTST+RK_QPOS+RK_IORF+RK_NI32+14);
      else ermark(RK_REQFERR);
      il->No.nsg = 0; }
   } /* End g2noise() */
...
*  been changed to agree with DECAY and CHANGE cards.  */
   ...
   while (kwret=kwscan(&ic,   /* Assignment intended */
         ...
         "SEED%RVIL", RP0->notmaster, &ppd->phiseed,
         "SIET%B20/27IL$mV",  &pdd->siet,
...
void g2refrac(struct CELLTYPE *il) {
   ...
   inform("(SW2=IH,B16IL,V>=0<1B30IL,B20/27IL$mV)",
      &prf->refrac, &prf->upsdst, &prf->omegadst, &prf->psdst, NULL);
   kwscan(&ic,
      "REFRAC%IH",            &prf->refrac,
      "UPSDST%B16IL",         &prf->upsdst,
      "OMEGADST%V>=0<1B30IL", &prf->omegadst,
      "PSDST%B20/27IL$mV",    &prf->psdst,
      "AHP%B20/27IL$mV",      &prf->ahp,
      "OPT%KIA",              &kode,
      NULL);
