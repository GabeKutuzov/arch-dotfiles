...
/* $Id: d3mset.c 44 2011-03-04 22:36:14Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                               d3mset                                 *
*                                                                      *
...
/* Allocate cross-response/correlation matrix (host and serial) */
   ...
   if (RP->kxr & (YSTATS|ZSTATS)) {
      size_t tmxn = (size_t)RP->mxnds;
      STAT.pxrmtx = (long *)mallocv((tmxn*(tmxn+1)>>1)*sizeof(long),
         "Cross-response matrix");
      }
...
*/
      ...
      for (pmdlt=RP->pmdlt1; pmdlt; pmdlt=pmdlt->pnmdlt) {
         ...
         pmdlt->pnr = (long *)mallocv
            (pmdlt->nds*sizeof(long),"Stim response counts.");
         } /* End modality loop */
...
/* Count total number of group masks needed */
   ...
   if (mgrpblk > STAT.lgrpblk) {
      if (STAT.lgrpblk > 0) free(STAT.pgrpblk);
   ...
   if (mgsblk > STAT.ngsblk) {
      if (STAT.ngsblk > 0) free(STAT.pgsblk);
