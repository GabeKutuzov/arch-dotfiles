...
/* $Id: d3mchk.c 51 2012-05-24 20:53:36Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                              d3mchk.c                                *
*                                                                      *
...
*  (2) Fill in modality request (rmdlts) and CTFXD (X,Y,Z,H,C,G stats  *
*      driven) bits in all cell types, propagating iteratively to all  *
*      connected target cell types.                                    *
...
void d3mchk(void) {
   ...
   long sumnds;               /* Total ncs+nds for broadcast */
      ...
      *  not just those that are actually entered in stats).  */
      ...
      for (ix=il->pct1; ix; ix=ix->pct) {
         ...
         *  assign a default according to the input type */
         ...
         case IA_SRC:         /* Input from the input array */
            ...
            *  MDLTFLGS_RQST implicitly set.  */
            {  int iwdw = ix->kgen & KGNST ? ix->srcnm.hbcnm : 0;
               ix->psmdlt = findmdlt("VISION", iwdw);
               } /* End IA_SRC local scope */
            break;
         ...
         case VW_SRC:         /* Input from window kinesthesia */
#ifdef D1
         case D1_SRC:         /* Input from Darwin 1 repertoire */
#endif
         ...
         *  amplification was requested, set the MDLTFLGS_KAMC bit.  */
         if (pmdlt = ix->psmdlt) {  /* Assignment intended */
            bitset((unsigned char *)&il->amdlts, (long)pmdlt->mdltno);
...
*  types that have H,X,Y,Z,C, or G stats. These may have been recog-
...
*  broadcast.  Check that sumnds does not exceed 8*SHRT_MAX, assuring
...
NoMarkPropagation:
   ...
   for (pmdlt=RP->pmdlt1; pmdlt; pmdlt=pmdlt->pnmdlt) {
      ...
      /* Calculate number of bytes needed to store nds bits */
      ...
      *  sense data broadcast.  */
      if (pmdlt->mdltflgs & MDLTFLGS_USED) {
         ...
         sumnds += pmdlt->nds;
         bitset((unsigned char *)&allmdlts, (long)pmdlt->mdltno);
         }
      ...
      if (pmdlt->mdltflgs & MDLTFLGS_CTFS) {
         pmdlt->pignc = (stim_mask_t *)callocv(2,
            BytesInXBits(pmdlt->ncs), "CTOPT=C check array");
   ...
   if (sumnds > SHRT_MAX)
      d3exit(LIMITS_ERR, "distinct stimuli", SHRT_MAX);
   RP->cumbcst += RP->cumnds = BytesInXBits(sumnds);
...
*  needed regardless of whether cell type has XYZHCG stats.  Also, set
*  the DIMDLT bit in cnflgs to indicate this case.  */
   ...
   for (il=RP->pfct; il; il=il->pnct) {
      ...
      if (il->nmdlts == 0) {
         if (il->Ct.kctp & KRPXRM) {
            cryout(RK_E1, "0***C,G,H,X,Y, OR Z STATS REQUESTED FOR ",
               RK_LN3, fmturlnm(il), LXRLNM, "    BUT NO MODALITY "
               "INFORMATION WAS FOUND", RK_LN0, NULL);
            il->Ct.kctp &= ~KRPXRM;    /* Kill other checking */
            }
         }
      /* Loop over conntypes, check for those with direct input */
         ...
         *  no modality was assigned (because globally not used) */
         ...
         /* If not from a repertoire, find bit offset */
         if (ix->kgfl & KGNXR || ix->cnflgs & OVMDLT) {
            register long mboff = 0;
            ...
            *  other than number order--not currently done. */
            for (pmdlt=RP->pmdlt1; pmdlt; pmdlt=pmdlt->pnmdlt)
               if (bittst((unsigned char *)&il->rmdlts,
                  (long)pmdlt->mdltno) && pmdlt->mdltno < mymdltno)
                  mboff += pmdlt->nds;
            ix->mdltmoff = mboff;
      ...
      *  no need to check that condition here).  */
      ...
      if (il->Ct.ctopt & OPTCFS && il->ctwk.tree.krmop) {
