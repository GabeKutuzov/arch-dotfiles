...
/* Declarations: */
   ...
   struct headermsg {
      int celltype;
      int nits;
      unsigned int seed; } *headermsg;

   struct cellmsg { 
      long cells;
      long conntypes;
      double delta; } *cellmsg;

   struct connmsg {
      long nconns;
      long source;
      long rule;
      double scale; } *connmsg;
...
/* from testtask */
   double t1, t2, lstart, lstop, tavg=0.0, ravg=0.0, wavg=0.0;
   long *lp, *lpAlloc;
   int msgIdx;
   MVPMSG_s *mp;


/* from testtask example */
   sprintf(buffer1,"SIP80 UM starting (pri=%ld)\n",TaskGetPriority());
   t1 = GICTimerGet();
   flush(buffer1,sizeof(buffer1));
   t2 = GICTimerGet();
   syslog(buffer1);

   xprintf("UM; flush(128) took %lf\n",(t2-t1));


   lp = (long *)memalign(sizeof(double),MVPMSG_MSGBUFW * sizeof(long));
   if(lp == NULL) {
	syslog("UM; buffer allocation failed");
	TaskExit();
      }
...
/*   headermsg = (struct headermsg *) MSGBUF(mp);
   ncelltypes = headermsg->celltype;
   niterations = headermsg->nits;
   seed = headermsg->seed;*/
   ...
   for (jl=1;jl<=ncelltypes;jl++) {
      ...
      sscanf(input[ptr++], "%ld %ld %f", &(il->ncells), &(il->nconntypes), &tdelta);
/*      cellmsg = (struct cellmsg *) MSGBUF(mp);
      il->ncells =  cellmsg->cells;
      il->nconntypes =  cellmsg->conntypes;
      tdelta =  cellmsg->delta;
   ...
   list memory better localized */
      ...
      for (jx=1;jx<=il->nconntypes;jx++)  {
         if ((ix=(struct CONNBLK *)malloc(sizeof(struct CONNBLK)))==NULL) {
            printf("No mem for CONNBLK"); exit(4);
            }
...
/*        mp = (MVPMSG_s *)hostReceiveMsg();
        if (mp == NULL) {
           syslog("UM; reception failed");
	   TaskExit();
	   }
        connmsg = (struct connmsg *) MSGBUF(mp);*/
        sscanf(input[ptr++], "%ld %d %d %f", &(ix->nconns), &(ix->source),
                                       &(ix->rule), &tscale);
/*        ix->nconns = connmsg->nconns;
        ix->source = connmsg->source;
        ix->rule = connmsg->rule;
        tscale = connmsg->scale;
        hostReclaimMsg(mp);*/
   ...
   for (il=play1;il!=NULL;il=il->play) {
      ...
      if ((il->prd=(struct CONNDATA *)malloc((unsigned int)
         il->ncells*il->nconntotal*sizeof(struct CONNDATA)))==NULL) {
           printf("nct %d ncells %d couldn't alloc cif lij\n",
...
/* Make fixed-point working thresholds */
   sprintf(buffer1, "after cellytypes loop\n");
   flush(buffer1,sizeof(buffer1));
   syslog(buffer1);
...
/*   time(&time3); */             /* Note ending time */
   ...
   printf("%d iterations required %lf seconds\n",
      niterations,time3-time2);

/* Report "DAS" statistics for final iteration only. */

   sprintf(buffer1, "DAS statistics for final iteration:\n");
   flush(buffer1,sizeof(buffer1));
   syslog(buffer1);
