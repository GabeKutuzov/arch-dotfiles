...
#include "dspAPI.h"
#include "host.h"
#include "mvpComm.h"          /* Host communications */
...
/* Macros for obtaining and sending host message buffers */
#define GetHMsg(msg)    (msg = TaskReceiveMsg(HMsgAvail))
#define SendHMsg(msg)   (TaskSendMsg(msg, HMsgQueue))
...
Port HMsgAvail;               /* Port for available host messages */
Port HMsgQueue;               /* Port for host msg sending queue */
...
void *mallocv(unsigned int n) {
   ...
   size_t nn = (n+63) & (~63);
   GetHMsg(msg);
   sprintf(msg, "malloc requesting %u bytes, rounded to %u\n", n, nn);
   SendHMsg(msg);
   mret = memalign(MVP_CACHE_SUBBLOCK, nn);
   GetHMsg(msg);
   sprintf (msg, "malloc returned %x\n", mret);
   SendHMsg(msg);
   return mret;
   }
#else
...
void rptlij(struct CELLBLK *play1) {
   ...
   for (jl=1,il=play1; il; il=il->play,jl++) {
      ...
      for (jx=1,ix=il->pct1; ix; ix=ix->pct,jx++) {
         GetHMsg(msg);
         sprintf(msg, "For celltype %d, conntype %d, first lij is %d,"
...
void prts(struct CELLBLK *play1) {
   ...
   for (jl=1,il=play1; il; il=il->play,jl++) {
      GetHMsg(msg);
      sprintf(msg, "si for celltype %d\n", jl);
      SendHMsg(msg);
      for (j=0; j<il->ncells; j+=10) {
         int k,kmax=min(il->ncells,j+10);
         char tdig[9];
         GetHMsg(msg);
         for (k=j; k<kmax; k++) {
            sprintf(tdig, "%8d", il->psi[k]);
            strcat(msg, tdig); }
         strcat(msg, "\n");
         SendHMsg(msg);
         }
...
TaskId HostMsgTask;           /* Id of host message task */
...
TaskId CleanUpTask[NumPP];    /* Ids of work unit cleanup tasks */
   ...
   Message msg;               /* Ptr to host message buffer */
...
*  to go astray.  */
   ...
   HMsgAvail = TaskOpenPort(-1);
   HMsgQueue = TaskOpenPort(-1);
   Load      = TaskOpenPort(-1);
   WBack     = TaskOpenPort(-1);

   HostMsgTask = TaskCreate(-1, HostCommun, NULL,
      HostMsgPrio, StackSize);
   TaskResume(HostMsgTask);
...
/* Read and echo header info for setting up loops */
   ...
   GetHMsg(msg);
   sprintf(msg, "Numbers of celltypes = %d, iterations = %d, seed = %u,"
...
/* Check for valid number of processors requested */
   ...
   if (nprocessors > NumPP) {
      GetHMSg(msg);
      sprintf(msg, "Too many processors\n");
      SendHMsg(msg);
      exit(2);
      }
...
/* Read cell type information and create cell block linked list */
   ...
   for (jl=1; jl<=ncelltypes; jl++) {
      if ((il=(CELLBLK *)mallocv(sizeof(CELLBLK)))==NULL) {
         GetHMsg(msg);
         sprintf(msg, "No mem for cellblk\n");
         SendHMsg(msg);
         exit(3);
         }
      ...
      GetHMsg(msg);
      sprintf(msg, "   Cell type %d\n",jl);
      SendHMsg(msg);
      GetHMsg(msg);
      sprintf(msg, "      Ncells = %ld, nconntypes = %ld, delta = %f\n",
   ...
   blocks in a linked list chained to the controlling CELLBLK.  */
      ...
      for (jx=1; jx<=il->nconntypes; jx++)  {
         if ((ix=(CONNBLK *)mallocv(sizeof(CONNBLK)))==NULL) {
            GetHMsg(msg);
            sprintf(msg, "No mem for CONNBLK\n"); 
            SendHMSg(msg);
            exit(4);
            }
         ...
         GetHMsg(msg);
         sprintf(msg, "      Connection type %d\n",jx);
         SendHMsg(msg);
         GetHMsg(msg);
         sprintf(msg, "         Nconns = %ld, source = %d, rule = %d,"
...
/* Complete the connection blocks:
   ...
   for (il=play1; il; il=il->play) {   /* Celltype */
      ...
      if (il->prd == NULL) {
         GetHMsg(Msg);
         sprintf(msg, "Nconns %d ncells %d couldn't alloc cif lij\n",
...
#ifdef DBGF
   GetHMsg(msg);
   sprintf(msg, "Calling rptlij first time\n");
   SendHMsg(msg);
   rptlij(play1);
#endif
...
/* Note the end-of-setup time */
   ...
   time2 = GICTimerGet();
   GetHMsg(msg);
   sprintf(msg, "Setup required %lf seconds\n", time2-time1);
   SendHMsg(msg);
...
#ifndef SIM  /***DEBUG***/
GetHMsg(msg);
sprintf(msg, "Completed task setup, starting PP setup\n");
SendHMsg(msg);
#endif
...
/* Print DAS results */
   ...
   for (jl=1,il=play1; il; il=il->play,jl++) {
      GetHMsg(msg);
      sprintf(msg, " Celltype %d \n", jl);
      SendHMsg(msg);
      for (jx=1,ix=il->pct1; ix; ix=ix->pct,jx++) {
         GetHMsg(msg);
         sprintf(msg, "      Conntype %3d: %8ld%8ld%8ld%8ld\n",
...
/*** DEBUG ***/
      GetHMsg(msg);
      sprintf(msg, "Dispatch reached end of iteration\n");
      SendHMsg(msg);
...
/***********************************************************************
*                                                                      *
*  HostCommun Proecess:  This process receives text messages from      *
*  other processes and sends them to the host for recording in         *
*  stdout.  Its raison-d'etre is to serialize access to the host.      *
*                                                                      *
*  N.B.  memalign() is used instead of mallocv() because latter may    *
*  not be reentrant.                                                   *
***********************************************************************/

void HostCommun(void *taskArg) {
   
   Message *msg;              /* Ptr to a message */
   int im;                    /* Counter for msg alloc */

#ifndef SIM                   /* No host communications in simulator */
   COMM_s *sendMsg,recvMsg;   /* Ptr to host control messages */
   MVPMSG_s *mp;              /* Ptr to a host text message */

   /* Initialize communication with the host */
   commNodeControl(CNC_REGISTER);
   commNodeControl(CNC_NOTIFY, NOTIFY_WAIT);

   sendMsg = (COMM_s *)memalign(sizeof(double), sizeof(COMM_s));
   recvMsg = (COMM_s *)memalign(sizeof(double), sizeof(COMM_s));

   /* Shake hands just to be sure all is well */
   mp = (MVPMSG_s *)hostReceiveMsg();
   if (!mp) TaskExit();
   if (MSGOP(mp) != CLIENT_INIT) TaskExit();
   if (hostReclaimMsg(mp) != 0) TaskExit();

   if (hostRouteMsg(SERVER_INIT, sizeof(COMM_s), sendMsg) < 0)
      TaskExit();
#endif

   /* Create a shared pool of text message buffers */
   for (im=0; im<NumHMsgs; im++) {
      msg = TaskAllocMsg(HMsgSize, HMsgAvail);
      TaskReclaimMsg(msg);
      }

   /* Act as a server to send messages to host */
   for (;;) {

      msg = TaskReceiveMsg(HMsgQueue);

#ifndef SIM                   /* No host communications in simulator */
      sendMsg->size = strlen(msg);
      sendMsg->addr = msg;
      if (hostRouteMsg(SERVER_MSG, sizeof(COMM_s), sendMsg) < 0)
         TaskExit();
      /* Wait while host copies over the text */
      mp = (MVPMSG_s *)hostReceiveMsg();
      if (!mp) TaskExit();
      if (MSGOP(mp) != CLIENT_DONE_MSG) TaskExit();
      if (hostReclaimMsg(mp) != 0) TaskExit();
#endif
