/* (c) Copyright 2013-2017, The Rockefeller University *11115* */
/* $Id: dsrsjqe.c 66 2018-03-15 19:01:23Z  $ */
/***********************************************************************
*                  ROCKS Library - C Implementation                    *
*                               dsrsjqe                                *
*                                                                      *
*  This function performs scaled and rounded division of a signed      *
*  32-bit dividend by an si32 divisor.  It first scales the dividend   *
...
*                                                                      *
*  Synopsis: si32 dsrsjqe(si32 x, int s, si32 y, int ec)               *
*                                                                      *
...
*                                                                      *
************************************************************************
*  V1A, 06/12/13, GNR - New routine, based on dsrswqe()                *
*  ==>, 06/12/13, GNR - Last date before committing to svn repository  *
*  Rev, 10/25/13, GNR - Eliminate 'L' constants                        *
*  R66, 12/25/17, GNR - No shift if s == 0 JIC                         * 
***********************************************************************/
...
si32 dsrsjqe(si32 x, int s, si32 y, int ec) {
   ...
   if (abs(s) > 63) { abexitq(75); return 0; }

   /* Compute sign of quotient, which will also be overflow
   *  return value  */
   sgnq = (x ^ y) < 0 ? ~SI32_MAX : SI32_MAX;
...
# else               /* System does not have I64 */
   ...
   /* Compute sign of quotient, which will also be overflow
   *  return value  */
   sgnq = (x ^ y) < (si32)0 ? ~SI32_MAX : SI32_MAX;
