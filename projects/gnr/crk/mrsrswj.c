...
/* $Id: mrsrswj.c 61 2017-01-16 19:26:44Z  $ */
/***********************************************************************
*                  ROCKS Library - C Implementation                    *
*                               mrsrswj                                *
*                                                                      *
*  This function adds rounding to mrsswj. It multiplies a 32-bit sign- *
...
*  specified right shift, and returns the 64-bit result.  Overflow is  *
*  not possible, so the mrsrswj[de] versions do not exist.             *
*                                                                      *
*  Synopsis: si64 mrsrswj(si32 x, si32 y, int s)                       *
*                                                                      *
...
************************************************************************
*  V1A, 09/27/14, GNR - New routine, based on msrsl                    *
...
si64 mrsrswj(si32 x, si32 y, int s) {
   ...
   prod = (si64)x * (si64)y;
   /* If there is no shift, there is no rounding either.  Use
   *  the fact that rounding is same for negs or positives.  */
   if (s)
      prod = SRA(prod + (SI64_01 << (s-1)), s);
   return prod;
...
*  64-bit code) for odd negative results, it is necessary to com-
...
/* Perform the round and shift */
   ...
   if ((ds = s - BITSPERUI32) >= 0) {
      ...
      if (ds == 0)
         ...
         p2.pu += 1UL << (ds-1), rv.lo = SRA(p2.ps, ds);
      }
   else {
      if (s) {
         ay = 1UL << (s-1);
         p2.pu += (~p0.pu < ay);
         rv.lo = ((p0.pu + ay) >> s) | (p2.pu << (-ds));
         rv.hi = SRA(p2.ps, s); }
      else
         /* If there is no shift, there is no rounding either */
         rv.hi = p2.ps, rv.lo = p0.pu;
      }
   return rv;
#endif
