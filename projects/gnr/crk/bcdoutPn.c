...
/* $Id: bcdoutPn.c 67 2018-05-07 22:08:53Z  $ */
/***********************************************************************
*                  ROCKS Library - C Implementation                    *
*                             bcdoutPn.c                               *
*                                                                      *
*  This is a version of bcdout, the ROCKS numerical output conversion  *
*  routine for floating-point numbers, that does not use any ROCKS     *
*  card input/print routines so it can be used in MATLAB mex files     *
*  and parallel computer PARn executables.  A descendent of long-ago   *
*  IBM Assembler versions by G.N. Reeke                                *
*                                                                      *
...
************************************************************************
*  V1A, 10/21/16, GNR - New program, based on bcdout()                 *
...
/*
   ...
   The operation code 'ic' is the sum of three quantities:
      ...
      Additionally, the globals RK.expwid and RK.length are accessed
         in bcdout().  The RK structure is replaced in wbcdwtPn and
...
*  STT, OUT_SIZE, UI32_SIZE, or LONG_SIZE change.  */
   ...
   double tensPn[STT+3] = {1E0, 1E1, 1E2, 1E3, 1E4, 1E5, 1E6,
...
*                         Hexadecimal output                           *
*                                                                      *
*  Now calls wbcdwtPn to eliminate redundant, rarely used, code.  The  *
...
*  (May call bcdoutPn recursively if goes to forced E format.)         *
*---------------------------------------------------------------------*/

#if RK_SNGL != RK_NI32
#error Recode bcdoutPn for case RK_SNGL != RK_NI32
#endif
   if (mode == RK_HEXF) {
      if (ic & RK_SNGL) {        /* Handle single precision float */
         float farg = (float)arg;   /* Coerce back to single */
         wbcdwtPn(&farg, field, ic); }
      else {                     /* Handle double precision float */
         wbcdwtPn(&arg, field, ic); }
      return;
...
*                            Octal output                              *
*                                                                      *
*  This is unlikely ever to be used, but by calling wbcdwtPn we can    *
...
*  output. (May call bcdoutPn recursively if goes to forced E format.) *
*---------------------------------------------------------------------*/
...
/* Force E format if data would overflow user's field */
   ...
   absarg = fabs(arg);
   if (davail < 0 || absarg >= tensPn[min(OUT_SIZE,davail)])
      goto force;
...
doEfmt:
   ...
   if (absexp > STT) {        /* Need power of ten outside table */
      ...
      factor = tensPn[taexp & (STT-1)];
      for (power = tensPn[STT]; ; power *= power) {
         if (taexp & STT) factor *= power;
         if ((taexp >>= 1) < STT) break; }
      }
   else factor = tensPn[absexp];
...
*  is used if RKPn.expwid was not preset, otherwise, the larger of
*  RKPn.expwid or the size actually needed is used.
*
...
*  with HAS_I64, bcdoutPn was found to give different answers from the
...
*  conversion modified accordingly.  */
   ...
   if (mode == RK_EFMT) {
      if (absarg >= tensPn[dec+(1-dgt0)]) {
         if (sadj) abexit(41);
         sadj = 1; goto doEfmt; }
      }
   else {
      if (absarg >= tensPn[min(davail, (OUT_SIZE+1))]) {
         if (autodec && dec > 1) {
...
*  the breakdown to high and low pieces.  */
   ...
   {  ui64 quot, warg = (ui64)absarg;
      do {
         quot = warg/10;
         *ld-- = dectab[warg - 10*quot];
         warg = quot;
         } while (warg);
      } /* End warg local scope */
#else
   {  ui32 high, low;
      if (absarg >= tensPn[UI32_SIZE_M1]) {
         char *rde = rd - expwid - UI32_SIZE_M1;
         high = (ui32)(absarg/tensPn[UI32_SIZE_M1]);
         low  = (ui32)(absarg - (double)high*tensPn[UI32_SIZE_M1]);
         do {
