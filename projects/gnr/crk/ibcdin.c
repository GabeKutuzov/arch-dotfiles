...
/* $Id: ibcdin.c 61 2017-01-16 19:26:44Z  $ */
/***********************************************************************
*                  ROCKS Library - C Implementation                    *
*                              ibcdin.c                                *
*                                                                      *
*  ROCKS signed integer numerical input conversion routines            *
...
*  Rev, 05/24/09, GNR - Split out from bcdin.c                         *
***********************************************************************/
...
/*
   For characters to long integer (or general fixed point):
   ...
   The operation code 'ic' is the sum of four quantities:
      ...
      'op' specifies the operation to be performed.  It is made
         ...
         0x00000800  RK_INT   Integer (IBCDIN only).  This bit has no
...
*                           Function IBCDIN                            *
*                                                                      *
*  This version of IBCDIN has been implemented as a call to DECIN      *
...
/* Handle hexadecimal input */
   ...
   if ((ic & RK_IORF) == RK_HEXF) {
      if (ic & RK_BYTE) {
         schr scval;
         hexin(ic, field, (byte *)&scval, sizeof(schr));
         ival = (si32)scval; }
      else if (ic & RK_SNGL) {
         short sval;
         hexin(ic, field, (byte *)&sval, sizeof(short));
         ival = (si32)sval; }
      else
         hexin(ic, field, (byte *)&ival, sizeof(si32));
      }
...
/* Decimal input.  First set up values needed for overflow tests */
      ...
      if      (ic & RK_BYTE) iint = (BITSPERBYTE-1),
         tval = (double)SCHR_MAX;
      else if (ic & RK_SNGL) iint = (HSIZE*BITSPERBYTE-1),
         tval = (double)SHRT_MAX;
      else                   iint = (BITSPERUI32-1),
         tval = (double)SI32_MAX;

      /* Use decin to get double precision value sans sign */
...
*  fraction that will be dropped later will not interfere.  */
      ...
      /* Restore sign */
      if (RK_Inflags & MINUS_FOUND) ival = -ival;
