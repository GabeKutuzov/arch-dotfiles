...
/* $Id: nmitrnaf.c 61 2017-01-16 19:26:44Z  $ */
/***********************************************************************
*                  ROCKS Library - C Implementation                    *
*      Nelder-Mead Simplex Optimizer without Simulated Annealing       *
*               (This is the single-precision version)                 *
*                                                                      *
*  This function package uses the Nelder-Mead simplex algorithm, as    *
*  described in the book "Numerical Recipes in C" (2nd Ed.)  by WH     *
*  Press, SA Teukolsky, WT Vetterling, and BP Flannery, Cambridge      *
*  University Press, 1992 with some ideas for program organization     *
*  borrowed from the fminsearch() program in MATLAB.  Versions with    *
*  and without simulated annealing are provided.  This is the one      *
*  without.                                                            *
*                                                                      *
*  See programming notes and references in the setup file, nelmeadf.c  *
*  The requirements for the user callback cost-evaluation function,    *
*  ufn, are described in the writeup for the startup routine, nminitf. *
*                                                                      *
...
*                              nmitrnaf                                *
*                                                                      *
*  Perform Nelder-Mead simplex optimization w/o simulated annealing    *
*                                                                      *
...
*  call to nmgetxf(), then nminitf() must be called to initialize      *
...
*              found in possibly a series of nmitrnaf() calls since    *
*              the last call to nminitf().                             *
*              N.B.  Swapping this vector back into the simplex        *
*              before another call to nmitrnaf() can in principle      *
...
*                                                                      *
*  Returns:    A return code defined in nelmeadf.h as follows:         *
...
*              in as described above.  nmgethf() can be called to      *
*              locate history records requested by the NMKH_LAST       *
*              or NMKH_EVRY option codes.  nmgetsf() can be called     *
*              to locate counts of the operations performed.           *
*              Finally, nmgetxf() can be called to locate the          *
...
int nmitrnaf(struct nmglblf *nmG, void *usrd, nmxyf **ppxbest,
      nmxyf *pybest, ui32 *pniter,
      nmxyf ftol, nmxyf xtol, ui32 mxiter) {

   nmxyf (*ufn)(nmxyf *x, void *usrd) = nmG->ufn;
   nmxyf  *xsums,*xr,*xt;        /* Ptrs to x sums,temp arrays */
   nmxyf  *xhi;                  /* Ptr to vertex w/highest y */
   nmxyf  *px,*pxe;              /* An x vector and its end */
   nmxyf  *px0 = nmG->nmasx;     /* Ptr to x array */
   nmxyf  *py0 = nmG->nmasy;     /* Ptr to y array */
   ui32   *stats = nmG->nmstats; /* Ptr to stats array */
   nmxyf  hftol = 0.5*ftol;
   nmxyf  xtol2 = xtol*xtol;
   nmxyf  fNi;                   /* Value of 1/N or 1/(N+1) */
   nmxyf  ar,ar1;                /* Avoidance ratio */
   nmxyf  rr0,rr1;               /* Reflection ratio */
   nmxyf  xr0,xr1;               /* Expansion ratio */
   nmxyf  oc0,oc1;               /* Outside contraction ratio */
   nmxyf  ic0,ic1;               /* Inside contraction ratio */
   nmxyf  sr1;                   /* Shrinkage ratio */
   nmxyf  thisy;                 /* y saved for history reporting */
   nmxyf  ybot,ytop;             /* Current worst, best y */
   nmxyf  ylo,yhi,ynhi;          /* Current low, high, next high y */
   nmxyf  yr,yt;                 /* Temp y, later radius values */
   ui32   h,htest;               /* Current cycle, last cycle */
   ui32   NN = nmG->N;           /* (nmallof assures N > 2) */
   ui32   NNf = nmG->N*sizeof(nmxyf);
   ...
   int    vlo,tlo,vhi;           /* Low, high vertices */

   /* Names of moves in same order as NMKM defines in nelmeadf.h */
   ...
   sr1 = nmG->nmassr;
   ar  = nmG->nmasar;
   ar1 = (1.0 + ar)/ar;    /* (nminitf() assures ar != 0) */
   h   = nmG->h;           /* Current cycle */
   htest = h + mxiter;

   /* Allocate space for history data (high-water-mark) */
   if (lopt & (NMKH_EVRY|NMKH_LAST) && htest > nmG->nhallo) {
      nmG->phist = (struct nmhistf *)reallocv(nmG->phist,
         htest*sizeof(struct nmhistf), "Optimization history");
      nmG->nhallo = htest;
      }
...
*   highest, and lowest (best) and the index of each.  */
      ...
      ybot = ylo, ytop = yhi;    /* Save for reporting */
...
*  never succeed.  The test on the x values (radius) is performed only
*  after a shrink or contract_in step, because it is rather expensive.
*  Unlike MATLAB, I do the OR of the two tests, not the AND.  I also
*  always do at least one cycle before testing in order to avoid
*  infinite call loops when nothing is changed.  (The ftol test is
*  better done here, rather than at the end as in the MATLAB version,
*  because it avoids the need to update ybot and ytop twice in each
*  cycle).  */

      if (dostop >= 0 && fabsf(ytop - ybot) < hftol *
            (fabsf(ytop) + fabsf(ybot))) {
         stats[NMS_CONVG] += 1;
         dostop = NM_CONVERGED;
         goto EndIteration2;
         }
...
/* Range 2:  ylo <= yr < ynhi <= yhi, accept reflection */
...
/* Range 3:  ynhi <= yr < yhi, try outside contraction */
...
/* Range 4:  All outside motions tested gave yt >= yhi
...
*  rather than have a test for iv==vlo inside the loop.  The y value
*  reported is the best value (the one not replaced).  This is
*  consistent with the other cases in that it is the one unique point
*  on the simplex that was treated differently from the rest.  This
*  value sticks out in any plot of yhist vs h, marking the shrinks.
*  Test code in V3I that shrank multiple times until certain criteria
*  were met has been removed in V4A:  Shrinking is very expensive and
*  it seems best to repeat all the vertex tests each time rather than
*  shrink with the simplified tests.  */
...
TryShrink:
      ...
      /* Keep track of best for best-ever test */
      ylo = NM_SING, tlo = 0;
...
#else
         convrt("(P3,5H For J1A80,'radius is ',J1VE9.6,'after ',"
...
EndIteration2:
      ...
      if (lopt & NMKH_EVRY) {
         struct nmhistf *ph = nmG->phist + h - 1;
         ph->y      = thisy;
         ph->ybest  = ybot;
         ph->yworst = ytop;
