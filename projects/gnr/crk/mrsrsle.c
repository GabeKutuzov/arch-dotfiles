...
/* $Id: mrsrsle.c 61 2017-01-16 19:26:44Z  $ */
/***********************************************************************
*                  ROCKS Library - C Implementation                    *
*                               mrsrsle                                *
*                                                                      *
*  This function adds rounding to mrssle. It multiplies a 32-bit sign- *
...
*  result, reporting an overflow if the result exceeds 32 bits.        *
*  This is the version that uses the error code passed as an argument. *
*                                                                      *
*  Synopsis: si32 mrsrsle(si32 x, si32 y, int s, int ec)               *
*                                                                      *
...
************************************************************************
*  V1A, 12/28/08, GNR - New routine, based on jm64sh and jmsw          *
*  ==>, 12/30/08, GNR - Last date before committing to svn repository  *
*  Rev, 09/27/14, GNR - Same neg results as SRA w/ or w/o 64-bit arith.*
***********************************************************************/
...
#define LM32 0xffffffff80000000LL
   ...
   if (s) {
      prod += SI64_01 << (s-1);  /* SRA may access prod twice */
      prod = SRA(prod,s); }
...
/* Perform the round and shift */
   ...
   if ((ds = s - BITSPERUI32) >= 0) {
      ...
      if (s > 0) {
         ...
         p2.ps = SRA(p2.ps, s); }
      /* There is an overflow if any of the bits p2 after scaling are
