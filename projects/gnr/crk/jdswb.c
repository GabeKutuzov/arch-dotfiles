...
/* $Id: jdswb.c 61 2017-01-16 19:26:44Z  $ */
/***********************************************************************
...
*  pointer in the argument list.  This routine terminates on all       *
*  errors--ues jdswbe() to record overflows via e64act().              *
*                                                                      *
*  Synopsis: si32 jdswb(si64 x, si32 y, si32 *pr)                      *
*                                                                      *
...
*  ==>, 11/03/08, GNR - Last date before committing to svn repository  *
*  Rev, 11/24/08, GNR - Use / and % if si64 == long long, add explicit *
*                       overflow and divide by 0 checks for HAS_I64    *
*  Rev, 12/01/12, GNR - Modify comment re addition of jdswbe()         *
...
#ifdef HAS_I64
   if (abs64(x) >= abs64((si64)y)<<31) {
      abexitmq(73,"Divide check."); return 0; }

#if LSIZE == 8
   {  ldiv_t qrm = ldiv(x, (si64)y);
      *pr = (si32)qrm.rem;
