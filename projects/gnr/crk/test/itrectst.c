...
int main() {
   ...
   setmf(NULL, "localhost", "ITERRECT TEST", NULL,
      1024, 0, '8', 0, 0);
   setmovie(1,0,0);
   ...
   pencol("YELLOW");
   InitRectangleIndexing(&R, 12, 12, 0, 0, 12, 12);
   kf = 1;
   while (GetNextPointInRectangle(&R)) {
   ...
   pencol("YELLOW");
   InitRectangleIndexing(&R, 12, 9, 2, 2, 10, 7);
   kf = 1;
   while (GetNextPointInRectangle(&R)) {
      float fx = x0 + xi*(0.5 + (float)R.ix);
      float fy = y1 - yi*(0.5 + (float)R.iy);
   ...
   pencol("YELLOW");
   InitRectangleIndexing(&R, 15, 12, 3, 2, 5, 8);
   kf = 1;
   while (GetNextPointInRectangle(&R)) {
      float fx = x0 + xi*(0.5 + (float)R.ix);
      float fy = y1 - yi*(0.5 + (float)R.iy);
   ...
   pencol("YELLOW");
   InitRectangleIndexing(&R, 15, 11, 2, 6, 12, 1);
   kf = 1;
   while (GetNextPointInRectangle(&R)) {
      float fx = x0 + xi*(0.5 + (float)R.ix);
      float fy = y1 - yi*(0.5 + (float)R.iy);
   ...
   endplt();
   cryout(RK_P1, "0End of iterrect test", RK_LN2, NULL);
   cryout(RK_P1, "\0", RK_LN0+RK_FLUSH+1, NULL);
   return 0;
   }
