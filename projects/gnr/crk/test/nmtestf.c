...
*           (single-precision, without simulated annealing)            *
***********************************************************************/
...
int main(void) {
   ...
   struct nmglblf *nmG;
   struct nmhistf *phist;
   float *pxbest,ybest;
   float *v0,*v;
   ui32 niter,*pstat;
   int nm;

   cryout(RK_P3, "0Begin Nelder-Mead (float) optimization test",
      RK_LN2, NULL);

   nmG = nmallof(Ndim, 0, myfn);
   v = v0 = nmgetxf(nmG);
   ...
   ybest = nminitf(nmG, NULL, "nelmead test",
      NMKA_MOVE | NMKP_EVRY | NMKP_DETAIL);

   convrt("(P3,'0The best initial function value is ',J0E12.6)",
      &ybest, NULL);

   nm = nmitrnaf(nmG, NULL, &pxbest, &ybest, &niter,
      1E-4, 1E-4, 1000);

   convrt("(P3,'0Refinement returned code ',J0I6)",
      &nm, NULL);
   convrt("(P3,' Solution found was at x = ',J0F12.6,' y = ',J0F12.6,"
      "' z = ',J0F12.6)", &pxbest[X], &pxbest[Y], &pxbest[Z], NULL);
   convrt("(P3,' Function value at solution was ',J0E12.6)",
      &ybest, NULL);
   if ((phist = nmgethf(nmG)) == NULL)
