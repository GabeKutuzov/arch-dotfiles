/* (c) Copyright 2009-2017, The Rockefeller University *11115* */
/* $Id: wbcdwt.c 66 2018-03-15 19:01:23Z  $ */
/***********************************************************************
*                  ROCKS Library - C Implementation                    *
*                              wbcdwt.c                                *
*                                                                      *
*  ROCKS 8-16-32-64-bit fixed-point numerical output conversion,       *
*  adapted from ibcdwt V3A.                                            *
*                                                                      *
*  ibcdwt V3A has the following features:                              *
...
*                                                                      *
*  Modifications from ibcdwt include the following:                    *
*  (1) 64-bit fixed-point types can now be handled.                    *
*  (2) The argument to be converted is now accessed via a pointer      *
*      so types of all lengths can be handled by the same routine.     *
*  (3) Order of call arguments changed so 'ic' comes last, possibly    *
*      eliminating a hole in the stack.                                *
*  (4) The RK_GFP bit becomes a new RK_NUNS bit to specify unsigned    *
*      arguments, eliminating the need for a separate routine.         *
*  (5) The codes specifying the argument length have been collected    *
*      in contiguous bits (0x1c0).  The old RK_PAD0 bit has been       *
*      moved to RK_NPAD0 to free up space for these new length bits.   *
*                                                                      *
...
*                                                                      *
************************************************************************
*  V3A, 01/03/09, GNR - Begin development from ibcdwt.                 *
*  ==>, 01/03/09, GNR - Last date before committing to svn repository  *
*  Rev, 08/11/09, GNR - Revise RK_NZ bits, add RK_Oct0, RK_NZNW        *
*  Rev, 09/01/11, GNR - Update expwid comments, add RK_PLUS, RK_LSPC   *
*  Rev, 09/07/11, GNR - Implement RK_NZNW as documented for octal      *
...
*  R66, 10/24/17, GNR - Bug fix -- LFTJ O/P blanked if field is full   *
***********************************************************************/
...
#ifdef HAS_I64
#if LSIZE == 8
   static ui64 Bigs[WIDE_SIZE-1] = { 9UL, 99UL, 999UL, 9999UL, 99999UL,
      999999UL, 9999999UL, 99999999UL, 999999999UL, 9999999999UL,
      99999999999UL, 999999999999UL, 9999999999999UL, 99999999999999UL,
      999999999999999UL, 9999999999999999UL, 99999999999999999UL,
      999999999999999999UL, 9999999999999999999UL };
#else
   static ui64 Bigs[WIDE_SIZE-1] = { 9ULL, 99ULL, 999ULL, 9999ULL,
      99999ULL, 999999ULL, 9999999ULL, 99999999ULL, 999999999ULL,
...
#endif
#elif BYTE_ORDRE < 0
   static ui64 Bigs[WIDE_SIZE-1] = { {9,0}, {99,0}, {999,0}, {9999,0},
      {99999,0}, {999999,0}, {9999999,0}, {99999999,0}, {999999999,0},
      {1410065407,2}, {1215752191,23}, {3567587327UL,232},
      {1316134911,2328}, {276447231,23283}, {2764472319UL,232830},
      {1874919423,2328306}, {1569325055,23283064},
      {2808348671UL,232830643}, {2313682943UL,2328306436UL} };
#else
   static ui64 Bigs[WIDE_SIZE-1] = { {0,9}, {0,99}, {0,999}, {0,9999},
      {0,99999}, {0,999999}, {0,9999999}, {0,99999999}, {0,999999999},
      {2,1410065407}, {23,1215752191}, {232,3567587327UL},
      {2328,1316134911}, {23283,276447231}, {232830,2764472319UL},
      {2328306,1874919423}, {23283064,1569325055},
      {232830643,2808348671UL}, {2328306436UL,2313682943UL} };
#endif
   /* Table of hex (and decimal) output characters */
   static char hextabuc[16] = "0123456789ABCDEF";
   static char hextablc[16] = "0123456789abcdef";
   /* Possible number prefixes and offsets vs control bits */
   static char pfx0[] = { " 0X+0X-0X 0x+0x-0x" };
   static char pfxo[] = { 1,3,0,3,6,6,6,6 };
...
#else
                  if (~targ.hi == 0 ||
                     targ.hi + (targ.lo > Bigs[l10].lo) > Bigs[l10].hi)
                        l10 += 1;
#endif
                  }
               else
...
#else
                     ~targ.hi == 0 || targ.hi +
                     (targ.lo > Bigs[texp].lo) > Bigs[texp].hi
#endif
                     ) {
...
*                         Hexadecimal output                           *
*                                                                      *
*  N.B.  ibcdwt treats all hex values as unsigned, but in wbcdwt it    *
*  is caller's choice--use RK_NUNS bit to treat as unsigned.  The      *
*  default behavior of hex output is to print the number of digits     *
...
*---------------------------------------------------------------------*/
      ...
      char *hextab = (ic & RK_NZLC) ? hextablc : hextabuc;
...
*                            Octal output                              *
*                                                                      *
*  N.B.  ibcdwt treats all octal hex values as unsigned, but in wbcdwt *
*  it is caller's choice--use RK_NUNS bit to treat as unsigned. Unlike *
...
*                                                                      *
*  N.B.  The RK_NPAD0 bit is quietly ignored--bcdout can't handle it,  *
*  and there is really no reason why it would be needed anyway.        *
*---------------------------------------------------------------------*/
...
/* Select fill character and store length of result */
   ...
   if (ic & RK_NPAD0) {
      fill = '0', RK.length = width;
      }
   else
      fill = ' ', RK.length = prd - pd + lpfx;
