...
/* $Id: nnputiz.c 3 2017-01-16 19:29:37Z  $ */
/***********************************************************************
*                          MPI Tools Library                           *
*                               nnputiz                                *
*                                                                      *
*     Write a single size_t to a message data stream, possibly not on  *
...
*                                                                      *
*  Swapping code from lemfmi8() is copied here to avoid call overhead. *
************************************************************************
*  V1A, 05/24/16, GNR - New routine, based on nnputul                  *
...
/* Be sure there is room in current buffer for FMZSIZE bytes
*  or else flush current buffer and start another one.  */

   char *m = nnputp(pnn, FMZSIZE);

#if BYTE_ORDRE > 0
/* System is big-endian, swapping is needed */
#ifdef HAS_I64
   /* Extend with zeros to 8 bytes if size_t is only 4 */
   register ui64 tu8 = (ui64)iz;
   m[0] = (char)tu8;
...
#else
   /* System does not have 64-bit arithmetic, so a size_t
   *  must be 32 bits.  Upper 32 are just zeros.  */
   register ui32 tu4 = (ui32)iz;
   m[0] = (char)tu4;
...
#if FMZSIZE > ZSIZE
   ui32 tu4 = 0;
   memcpy(m, (char *)&iz, ZSIZE);
   memcpy(m+4, (char *)&tu4, ZSIZE);
#else
   memcpy(m, (char *)&iz, ZSIZE);
#endif
