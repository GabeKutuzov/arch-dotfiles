...
/* $Id: allocpm.c 3 2017-01-16 19:29:37Z  $ */
/***********************************************************************
*                          MPI Tools Library                           *
*                               allocpm                                *
*                                                                      *
*  This routine is the malloc() analog in a shared memory management   *
*  package for parallel computers.  On serial computers, it is similar *
*  to traditional malloc() except for maintaining a running total of   *
...
*                                                                      *
*  Synopsis:                                                           *
*     void *allocpm(memtype kmem, si32 mtype)                          *
*                                                                      *
*  Arguments:                                                          *
...
/* Allocate mblkdef and storage together to save system calls */
   ...
   if ((pa = (struct mblkdef *)malloc(ltot)) == NULL)
      return NULL;
...
#ifdef PAR
   pa->info.magic = MPMAGIC | MPM_ADD | (intptr)kmem;
#else
   pa->info.magic = MPMAGIC | (intptr)kmem;
#endif
