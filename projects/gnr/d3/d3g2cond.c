/* (c) Copyright 2004-2005, The Rockefeller University *11115* */
/* $Id: d3g2cond.c 70 2017-01-16 19:27:55Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                             d3g2cond.c                               *
*                                                                      *
...
*                                                                      *
*  N.B.  Setting of cpspov left for redesign                           *
*                                                                      *
      ...
      for (jl=il1; jl!=il2; jl=jl->pnct) {
         ...
         for (pcnd=jl->pcnd1; pcnd; pcnd=pcnd->pncd) {
            if (hnewcdnm == pcnd->hcdnm) {
               CPTP.qcnd = pcnd;
               ktdcy = qdecay(&pcnd->gDCY) ? pcnd->kcond : CDTP_NDECAY;
               ...
               if (g3cptest(CDKY_DCY,                    /* DECAY */
                     qdecay(&pcnd->gDCY) == qdecay(&pcnn->gDCY),
                     "DECAY"))
                  pcnd->gDCY = pcnn->gDCY;
...
void getion(struct REPBLOCK *ir, struct CELLTYPE *il) {
   ...
   if (RP0->ccgrp & CCGRP3) {    /* Processing in Group III? */
      ...
      if (!ir) d3exit(fmturlnm(il), IONREG_ERR, 0);
      /* Assignment intended in for condition */
