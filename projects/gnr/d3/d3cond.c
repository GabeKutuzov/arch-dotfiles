/* (c) Copyright 2005-2007, The Rockefeller University *11115* */
/* $Id: d3cond.c 76 2017-12-12 21:17:50Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                               d3cond                                 *
*                                                                      *
...
/* Get last hit information if present */
      ...
      if (pcnd->cdflgs & CDFL_HITNT) {
         d3gtjl2(nhits, pr+ognh);
         d3gtjl2(tlhit, pr+ogth);
         }
...
/* N.B.  Be sure that CDAT.praffs has been set from CDAT.pfaffs before
*  this routine is called (this routine does not access ix->nct).  */
            {  si64 td = jrsl(CDAT.praffs[pcnd->iact-1],pcnd->uga.at);
               GotHit = qsw(td) >= 0; }
            break;
...
NoHitsNow:  ;
      ...
      if (!KeepEvent) d3ptjl2(zero, pr+ognh);
