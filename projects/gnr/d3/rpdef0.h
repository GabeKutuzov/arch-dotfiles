...
/* $Id: rpdef0.h 79 2018-08-21 19:26:36Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                        RPDEF0 Header File                            *
*                                                                      *
...
#ifdef PAR0                   /* psfcst is in lijarg for !PAR,PARn */
   si32            *psfcst;   /* Ptr to STGH cos/sin table (S16) */
#endif
   void            *ptvrsi;   /* Ptr to camera rescale info|histo */
...
*  input routines to be called.  */
   ...
   long cumntv;               /* Cumulative size of TV images (bcst) */
   long cumntvi;              /* Images for TV_Scale, TV_Filt (PAR0) */
   long cumnvbc,cumnvfc;      /* Cumulative size of byte,float senses */
   long cumval;               /* Size of all VDTDEF blocks */
   ulng cumsws;               /* Size of shared work space (Node 0) */
   ulng nbsi32;               /* Number of broadcast si32 values */

/* Memory allocations estimated globally in d3allo for d3echo,
*  then recomputed locally for each node in d3nset. */

   ui64 ldecay;               /* Space for decaying connections */
   ui64 lgall;                /* Bytes of group-level ion data */
   ui64 li0bxs;               /* Length of KPLEDG boxsums + strips */
   ui64 liall;                /* Length of all GCONN space needed */
   ui64 lrall;                /* Bytes of repertoire space needed */
   ui64 lsall;                /* Length of s(i) data for all reps */
#ifdef PAR
   ui64 ls0all;               /* Length of s(i) data on host node */
#endif
...
/* Miscellaneous parameters and derived quantities */
   ...
   struct DEFTHR   GDfD;      /* Global deferred params */
   struct DEFTHR   LDfD;      /* Local deferre params */
...
#define NVCOLS 2              /* Number of value columns */
   ...
   long pltbufl;              /* X graphics plot buffer length */
   long rprecp;               /* Packing factor for replay file */
   long rpirec;               /* Current record in replay file */

   size_t rprecl;             /* Record length of replay file */

   orns ilseed;               /* Random number seed for ilists */
#ifdef PAR0
   ui32 lknprt;               /* Length of kernel print data */
#endif
   ...
   int ljnwd;                 /* Length of joints and windows changes */
   ...
   ui16  mxnib;               /* Max value of nib over all gconns */
   ui16  mxtvixy;             /* Max of tvix,tviy over all cameras */
...
#define N0F_TVRSCL   0x0040   /* Some camera has rescale option */
#define N0F_TVRSCC   0x0080   /* Some camera has TV_RSCC option */
#define N0F_MKTBLS   0x0100   /* Make activation tables in d3ctbl() */
#define N0F_SPMFTM   0x0200   /* SPMFTIMER option entered */
#define N0F_SVCIJ0   0x0400   /* Save CIJ0 for later reset type 5 */
...
/* The rlvl array gives the reset actions to be performed at
*  various times.  The use of an array rather than individual
*  variables allows them to be set by common code in d3g3rset(),
*  where the order of option keys must match the order given in
*  the ReLops enum (see d3global.h).  The extra rlvl values are
*  for lvtuse and lvtdef, the used & deferred trial reset levels.  */
#define RLVNUM 8              /* Number of level suboptions */
   schr rlvl[RLVNUM+2];
...
*  being passed to the environment package. */
   ...
   ui16  kux;                 /* Stimulus pixels used - x */
   ui16  kuy;                 /* Stimulus pixels used - y */
