...
/* $Id: d3pdpz.c 77 2018-03-15 21:08:14Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                               d3pdpz                                 *
*                                                                      *
...
*  R67, 10/11/16, GNR - Receive and print CONNDATA, handle DPOOB better*
...
void d3pdpz(void) {
   ...
   si32 *pd32,*pd32inh;    /* Ptr to 32-bit type-detail items */
   si32 *pe32;             /* Ptr to end of type-detail items */
   si32 pitm;              /* Value to be printed */
...
/* Process all DP cells in current cell layer */
   ...
   while ((cellno = (int)ilstiter(&dpit)) >= 0) {
      if (cellno >= il->nelt) break;
...
*  Warning:  Ad-hoc code is used here to swap or not swap just part
*  of the larger CDAT structure.  This really should be done by
*  making a little inner structure and using nncom with an nxdr2
*  table.  */

      {  char nxdata[lnndat];
         char *pd = nxdata;
         ui32 *pl;
         nnget(&dpio, nxdata, (int)lnndat);
         for (pl=&CDAT.cdcell; pl<=(ui32 *)&CDAT.new_vm;
            pl++,pd+=FMJSIZE) *pl = lemtoi4(pd);
         CDAT.groupx = lemtoi2(pd), pd += HSIZE;
         CDAT.groupy = lemtoi2(pd), pd += HSIZE;
         memcpy((char *)&CDAT.old_phi, pd,
            &CDAT.flags - &CDAT.old_phi + 1);
         } /* End local scope */
   ...
   *  and used, otherwise a,b.  */
   if (taitms & DPABCD && il->Ct.rspmeth >= RF_IZH3) {
      ...
      if (il->prfrc) {                 /* DST term */
         if (CDAT.dst < DST_CLK0S16) { /* DST encodes refrac time */
...
*---------------------------------------------------------------------*/
      ...
      /* Collect all the data except vmvt & bandsums */
      pe32 = CDAT.pAffD32 + il->nAff32;
      for (pd32=CDAT.pAffD32; pd32<pe32; pd32++)
         *pd32 = nngeti4(&dpio);
      ...
      for (ix=il->pct1; ix; ix=ix->pct) {
         ...
         if (ix->Cn.kam & KAMCG) {
            si32 vmvt = nngeti4(&dpio);
            testput("(T,' V=',B" qqv(FBvl) "IH8.5X)", &vmvt);
            }
         } /* End ix loop */
      ...
      for (ib=il->pib1; ib; ib=ib->pib) {
         ...
         for (jx=0; jx<ib->nib; jx++) {                  /* Bandsums */
            si64 bsum = nngeti8(&dpio);
            if (jx > 0) testput("(TB20/27IW11.<4X)", &bsum);
            else        affput("BG1 ", &bsum, jct, AP64);
            } /* End loop over bands */
...
*  code is ignored because multi-row output would be hard to read.  */
      ...
      if (il->pct1 && taitms & (DPLIJ|DPDIJ|DPCIJ|DPCIJ0|DPMIJ|
            DPRIJ|DPPPF|DPSJ|DPDCIJ|DPTERM)) {

         struct CONNDATA *pND,*pD;  /* Ptrs to connection data */
         gvin_fn sjrget;            /* Fn to get Sjraw colors */
         ...
         int doSjraw;               /* Pix sz to print raw colors */
...
*  J, Lij, Dij, Cij, Cij0, Mij, Rij, DCij, PPF, Sj, and Term data */

            nuk = nngetil(&dpio);
            pND = CDAT.pconnd + ix->ocdat;
            for (isyn=0; isyn<nuk; isyn++) {

               /* First get and check Sj */
               if (doSj) {
                  char *tSj = text + doSj;
                  si32 sjval = nngeti4(&dpio);
                  if (sjval == MISSING_SJ) {
                     ...
                     /* Sj as used */
                     wbcdwt(&sjval, tSj, kbcds);
                     /* Optional phase */
                     if (ix->phopt & (PHASJ|PHASR|PHASC)) {
                        si32 phi = nngeti4(&dpio);
                        wbcdwt(&phi, tSj+DPCONNWIDTH,
                           RK_IORF+RK_NI32+DPPHIWIDTH-1);
                        }
                     /* Optional raw color in R-G-B format */
                     else if (doSjraw) {
                        struct EXTRCOLR *pxc = &ix->cnxc;
                        byte sjraw[HSIZE*NColorDims];
                        si32 rgb;
                        int ec;
                        nnget(&dpio, sjraw, doSjraw);
                        tSj += DPCONNWIDTH;
                        for (ec=0; ec<=2; tSj+=4,++ec) {
                           pxc->tveoff = (byte)ec;
                           rgb = 100*sjrget(sjraw, pxc);
                           wbcdwt(&rgb, tSj,
                              FBim<<RK_BS|RK_IORF|RK_NI32+3);
                           }
                        }
                     } /* End else normal Sj */
                  } /* End if doSj */

               /* Receive CONNDATA now (if needed for any variables
               *  below).  (This call increments pND for us.)  */
               if (doDCij|doTerm) nncom(&dpio, (void **)&pND,
                  ttloc(IXstr_CONNDATA), NNC_Rcv);
               pD = pND - 1;
               ...
               *  skipped connections is passed from comp node.  */
               ...
               /* Get and print Lij */
               if (doLij) {
                  long lij = nngetil(&dpio);
                  wbcdwt(&lij, text+doLij,
               ...
               if (doCij) {
                  pitm = nngeti4(&dpio);
                  wbcdwt(&pitm, text+doCij, RK_MZERO+(31<<RK_BS)+
               ...
               if (doCij0) {
                  pitm = nngeti4(&dpio);
                  wbcdwt(&pitm, text+doCij0, RK_MZERO+(31<<RK_BS)+
                     (4<<RK_DS)+RK_IORF+DPCONNWIDTH-1);
                  }
               ...
               if (doDCij|doTerm) {
                  wbcdwt(&pD->delCij, text+doDCij, (FBCij<<RK_BS)+
                     (4<<RK_DS)+RK_IORF+RK_NI32+DPCONNWIDTH-1);
                  }
               ...
               if (doTerm) {
                  sconvrt(text+doTerm,
                     "(2*B7/14IJ8.<4,B" qqv(FBvl) "IJ8.<4,B10IJ8.<4,"
                     "2*B16IJ8.<4)",
                     &pD->simmti, &pD->mjmmtj, &pD->vmvt,
                     &pD->wayscl, &pD->Cijprod, &pD->Cijterm, NULL);
                  }

               /* Print connection number */
               wbcdwt(&J, text, RK_IORF+RK_NLONG+DPCONNWIDTH-1);
            ...
            if (pitm != END_OF_GROUP)
               d3exit(fmturlnm(il), DPSYNC_ERR, jct);
