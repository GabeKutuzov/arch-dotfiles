/* (c) Copyright 1993-2017, The Rockefeller University *11116* */
/* $Id: findvc.c 74 2017-09-18 22:18:33Z  $ */
/***********************************************************************
*                             CNS Program                              *
...
*  R74, 05/28/17, GNR - Update getcshm for 16 bit gray or color modes  *
***********************************************************************/
...
static char *colnms[NColorSels] = { "BW", "RED", "GREEN", "BLUE",
   "R-G", "G-B", "B-R", "G-R", "B-G", "R-B" };
...
*                               getcshm                                *
*                                                                      *
*  This routine analyzes the color requested for a connection type and *
*  the nature of the data source and returns indexes or shift and mask *
*  to isolate that color (or colors) as described below.               *
*                                                                      *
*  Argument:                                                           *
*     pxtc     Ptr to an EXTRCOLR structure that has the user color    *
*              selector and source color mode and which receives the   *
*              output shift, mask, and excit/inhib offset variables.   *
*     emsg     Ptr to an indentifying string that can be inserted in   *
*              any error message.                                      *
*                                                                      *
*  Return values (in EXTRCOLR struct at *pxtc) when image is:          *
*  BM_BW mode:  Return without doing anything.  This mode is not       *
*              currently used in CNS and would be an error, but there  *
*              is no current way it can be turned on so no harm in     *
*              treating it as as an uninitailized default (BM_BW = 0). *
*              If we decide to support this mode, either it or uninit  *
*              must be made different from 0.                          *
*  BM_COLOR mode:  Error, this mode is currently not used internally.  *
*  BM_GS or BM_GS16 mode:  All zeros.  An error is generated if a      *
*              color value is requested.                               *
*  BM_C24 or BM_C48 mode:  tveoff,tvioff contain the indexes of the    *
*              selected excitatory,inhibitory (if used) colors in the  *
*              image data where 0 = red, 1 = green, 2 = blue.  tvshft  *
*              and tvmask are not used.                                *
*  BM_C16 or BM_C8 mode:  For the three colors in order red, green,    *
*              blue, tvshft is the shift needed to shift data of that  *
*              color to S14 position (BM_C08) or S15 position (BM_C16) *
*              and tvmask is the mask needed to isolate the selected   *
*              color before shifting (BM_C8 only).  tveoff and tvioff  *
*              select the excit,inhib shifts and masks.  tvrndc is the *
*              amount to be added for rounding BM_C8 color->gray cal-  *
*              culations, normal or COMPAT=R version.  These values    *
*              are currently (and probably forever) not in use, but    *
*              are kept here in case needed for code in d3imgt to      *
*              read such images and convert to BM_C24.                 *
*  tvlpxgc is set to the size of the processed image pixels, that is,  *
*              1 or 2 for gray or gray-from-color, 3 or 6 if color.    *
*  If gray values are requested from a colored image, the EXGfmC bit   *
*              is set.                                                 *
*  If the EXTR3C bit is set, the three colors are cycled through in    *
*  cyclic permutation order starting with the tveoff,tvioff colors.    *
*---------------------------------------------------------------------*/

void getcshm(struct EXTRCOLR *pxtc, char *emsg) {

   int ktvc = pxtc->tvcsel & EXMASK;
   int mode = pxtc->tvmode & BM_MASK;

   /* Values for pxtc->tvshft in RGB order.  N.B.  The BM_C16 shifts
   *  are to S15 because otherwise a negative shift would be needed
   *  for red.  The routines shift right one to S14.  */
   static const byte xtshftC8[]  = { 11, 8,  6 };  /* BM_C8 mode */
   static const byte xtshftC16[] = {  0, 5, 10 };  /* BM_C16 mode */
   /* Values for pxtc->tvmask arranged by color (BM_C8 mode only) */
   static const byte xtmaskC8[]  = { 0x07, 0x38, 0xc0 };
   /* Values for excit color for ColorSens choices (012=RGB) */
   static const byte xtcexc[] = { 0, 0, 1, 2, 0, 1, 2, 1, 2, 0 };
   /* Values for inhib color for ColorSens choices (012=RGB) */
   static const byte xtcinh[] = { 0, 0, 1, 2, 1, 2, 0, 0, 1, 2 };

   /* Zero everything that might not be used */
   pxtc->tveoff = pxtc->tvioff = 0;
   memset(pxtc->tvshft, 0, NColorDims);
   memset(pxtc->tvmask, 0, NColorDims);

   /* Set pixel size from color mode (before gray modes return!) */
   pxtc->tvlpxgc = (ui16)bmpxsz(mode);

   /* Set bits that control for the image mode */
   switch (mode) {
   case BM_BW:                /* Unused Bitmap or uninitialized mode */
      return;
   case BM_COLOR:             /* Invalid indexed color */
      cryout(RK_E1, "0***BITMAP AND INDEXED COLOR MODES NOT "
         "SUPPORTED FOR Sj INPUTS FOR ", RK_LN2, emsg, RK_CCL, NULL);
      return;                 /* Ignore ColorSens color request */
   case BM_GS:                /* Image is gray scale */
   case BM_GS16:
      /* Error if color requested from gray image */
      if ((enum ColorSens)ktvc != BandW)
         cryout(RK_E1, "0***CANNOT GET COLOR FROM A B&W SOURCE FOR ",
            RK_LN2, emsg, RK_CCL, NULL);
      return;                 /* Ignore ColorSens color request */
   case BM_C8:                /* Image is in old 2-3-3 color mode */
      memcpy(pxtc->tvshft, xtshftC8, NColorDims);
      memcpy(pxtc->tvmask, xtmaskC8, NColorDims);
      break;                  /* Deal with color selectors */
   case BM_C16:               /* Image is in 5-5-5 color mode */
      memcpy(pxtc->tvshft, xtshftC16, NColorDims);
      break;                  /* Deal with color selectors */
   case BM_C24:               /* Image is 24-bit color */
   case BM_C48:               /* Image is 48-bit color */
      break;                  /* Deal with color selectors */
      } /* End first-lvel mode switch */

   /* Set bits that select the requested colors.
   *  Note:  only color modes get to this code.  */
   if ((enum ColorSens)ktvc == BandW) {
      pxtc->tvcsel |= EXGfmC;
      /* (Call bmpxsz again--JIC getcshm called more than once) */
      pxtc->tvlpxgc = (ui16)(bmpxsz(mode)/NColorDims); }
   pxtc->tveoff = xtcexc[ktvc];
   pxtc->tvioff = xtcinh[ktvc];
   pxtc->tv3cinc = pxtc->tvcsel & EXTR3C ? 1 : 0;
   pxtc->tvrndc = RP->compat & OLDIARND ? 0x2003 : 3;
...
*  Note that '*' indicating self-reference will not match and will
*  return 0, indicating REPSRC.  */
   static char *bltsnm[NRST-1] = { "IA", "V", "VJ", "VH", "VW",
      "VS", "VT", "TV", "PP", "VITP",
      };
...
*     from 1.  By convention, in R66 and later, this number is set to  *
*     1 each time a '-' code is entered for VJ,VT,VW.  For user-       *
