...
/* $Id: d3grp3.c 76 2017-12-12 21:17:50Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                               d3grp3                                 *
...
*  Rev, 06/07/12, GNR - Remove RESPFN card -- adefer now per conntype  *
*  Rev, 08/16/12, GNR - Use mdltbid to eliminate mdltno bit tests      *
*  Rev, 04/23/13, GNR - Add NOUP=R, move codes to RP->knoup            *
*  R70, 01/16/17, GNR - Change outnow states to bits                   *
*  R78, 03/24/18, GNR - Bug fix:  Make error if DETAIL and bad rlname  *
...
*---------------------------------------------------------------------*/
         ...
         case CYCLE:  {
            ...
            ui16 lntcs;
            ui16 lntrs;
            /* INTEGRATE options -- easy to add more later */
            static char *pino[] = { "0", "EULER" };
            ...
            *  Note: "EPL" == "PLOT" for downward compatibility.  */
            ...
            /*  N.B.: See note in rpdef.h re defs of kevtr bits.  */
               ...
               case 4:  /* SPMFTIMER */
                  ...
                  break;
               case 5:  /* INTEGRATION */
                  mret = match(RK_EQCHK, RK_MREQF,
                     ~(RK_COMMA|RK_ENDCARD), 0, pino, 2);
                  /* All mismatches already gave match() errors */
                  switch (mret) {
                  case 1:  RP->kinteg = 0; break;
                  case 2:  RP->kinteg = KIEUL; break;
                     }
                  break;
...
*  R78, 03/24/18, GNR - Make an error if unrec keyword is not an rnln  *
*                                                                      *
...
*---------------------------------------------------------------------*/
         ...
         case DETAIL:  {
            ...
            enum eops { DPUnkn=0, DPClist, DPPrint, DPTimer, DPDis,
               DPEnb, DPNew, DPAdd, DPDel, DPOff, DPExplic } emret; 
            static char *dtl[] = { "CLIST", "PRINT", "TIMER",
               "DISABLE", "ENABLE", "NEW", "ADD", "DEL", "OFF" };

            cdscan(card, 1, SCANLEN, RK_WDSKIP);

            while ((smret = match(RK_NMERR, RK_MNEWK,
                  ~(RK_COMMA|RK_EQUALS|RK_INPARENS|RK_LFTPAREN),
                  0, dtl, (int)DPOff)) != RK_MENDC) {
               emret = (enum eops)smret;

               /* If in parens, this has to be a cell list.  Reset
               *  emret so that if a region name happens to be the
               *  same as one of the recognized keywords, we will
               *  treat it as a cell list and not as one of those
               *  keywords.  */
               if (RK.scancode & RK_INPARENS) emret = DPExplic;

               /* Unrecognized keyword:  It could be an old-style snln
               *  otherwise it is an error.  */
               else if (emret == DPUnkn) {
                  /* Use a fake rlnm for checking */
                  rlnm tln;
                  scanagn();
                  setrlnm(&tln, NULL, NULL);
                  d3rlnm(&tln);
                  if (findln(&tln)) emret = DPExplic;
                  else {
                     ermark(RK_IDERR);
                     goto NEXT_CARD;
                     }
               ...
               case DPOff:          /* OFF */
                  ...
                  *  remove references to all cell lists.  */
                  if (scan(lea, 0) & RK_ENDCARD) {
                     /* First zero out the ctclloc's (which will
                  ...
                  /* Otherwise, drop through to case DPExplic ... */

               case DPNew:          /* NEW -- handled by d3clst */
               case DPAdd:          /* ADD -- handled by d3clst */
               case DPDel:          /* DEL -- handled by d3clst */
               case DPExplic:       /* Possible explicit cell list */
                  scanagn();
                  if (tdpd.dpclloc.clnum >= 0)
                     tdpd.dpclloc.clnum = --RP0->nextclid;
                  pclh = d3clst(tdpd.dpclloc.clnum, CLKM_INDIV);
                  if (pclh) {
               ...
               case DPClist:        /* Reference to numbered CLIST */
                  tdprm = getclid(&tdpd.dpclloc);
                  /* Find any existing DPDATA block for this clnum */
                  for (ppdpd=&RP0->pdpd1;
                        pdpd=*ppdpd;   /* Assignment intended */
                        ppdpd=&(*ppdpd)->pndpd) {
                     if (pdpd->dpclloc.clnum == tdpd.dpclloc.clnum) {
                  ...
                  pdpd = *ppdpd = mallocv(sizeof(struct DPDATA),
                     "Detail print data");
                  pdpd->pndpd = NULL;
                  /* Store parameters for this clloc in DPDATA.
            ...
            *  to all layers and all private DPDATA blocks. */
            if (nocl) {
               ...
               for (il=RP->pfct; il; il=il->pnct) {
                  if (il->ctclloc[CTCL_DPRINT].clnum) {
                     if (tdpd.jdpt > 0)
                        il->jdptim = tdpd.jdpt;
                     if (tdpd.mckpm >= 0)
                        mcodoph(&il->dpitem);
            ...
            *  simplicity, do check even if there was no change.)  */
            for (iei=EXCIT; iei<NEXIN; ++iei) {
               if (pea->pefil[iei] && pea->pect[iei])
                  ilstchk(pea->pefil[iei], (long)pea->pect[iei]->nelt,
                     eimsg[iei]);
               }
            break;
...
*---------------------------------------------------------------------*/
         ...
         case ENVVAL: {
            ...
            for (pmdlt=RP->pmdlt1; pmdlt; pmdlt=pmdlt->pnmdlt) {
               ...
               if (!(wmdlts & pmdlt->mdltbid)) continue;
...
*---------------------------------------------------------------------*/
         ...
         case SNOISE: {
            ...
            *  acrobatics with the char buffer, etc. */
            ...
            orns seedbuf = -1L;     /* Binary buf for seed val */
            orns msd;

            /* Clear SNFRBUF before inform */
            snfrbuf[0] = '\0';
            inform("(SW1NVB24IJ,VB28IJ,A" qDML ",V>IJ)",
...
*---------------------------------------------------------------------*/
         ...
         case VALUE: {
            ...
            kwscan(&ic,
               ...
               "BEST%RVI", (int)(ivb->kval<=VB_FIRE), &ivb->vbest,
               "HWIDTH%RVI",(int)(ivb->kval<=VB_UJMD), &ivb->vhalf,
