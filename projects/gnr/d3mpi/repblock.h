...
/* $Id: repblock.h 74 2017-09-18 22:18:33Z  $ */
/***********************************************************************
*                             CNS Program                              *
...
*  V8I, 04/03/13, GNR - Move vxo,vyo to RPDFLT area                    *
*  R65, 01/02/16, GNR - ng[xy] to ui16, ngrp to ui32, ncpg to ubig     *
*  R72, 03/08/17, GNR - Remove KRPRR,KRPSP, add xgap,ygap              *
***********************************************************************/
...
struct RPDFLT {
   ...
   float xgap,ygap;           /* Layer plot group % gaps */
...
/* Following bits are not options, but are set in d3news() | d3nset() */
#define KRPRG     0x1000         /* Some celltype has regeneration    */
...
struct REPBLOCK {
   ...
   ui64 lrt;                  /* Len of repertoire - total bytes  */
   ulng lgt;                  /* Len of group data for all groups */
   ubig ncpg;                 /* No. of cells (all types) per gp */
   char sname[LSNAME];        /* Short name of repertoire */

   float aplx;                /* Plot X origin (left edge) */
   float aply;                /* Plot Y origin (bottom edge) */
   float aplw;                /* Plot width */
   float aplh;                /* Plot height */
   ui32  ngrp;                /* No. of groups in repertoire */
   int   nqx,nqy;             /* Rows/cols to plot one group */
   ui16  andctopt;            /* AND of all ctopt bits (after d3news) */
   ui16  andctkst;            /* AND of all kstat bits (after d3news) */
   ...
   ui16  mxnib;               /* Max(nib) over child gconns */
   ui16  ngx;                 /* No. of groups along X */
   ui16  ngy;                 /* No. of groups along Y */
   ui16  nlyr;                /* Number of layers (cell types) */
