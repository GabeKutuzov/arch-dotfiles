...
/* $Id: d3elij.c 70 2017-01-16 19:27:55Z  $ */
/***********************************************************************
*                            CNS Program                               *
*                          d3elij routines                             *
*                                                                      *
...
*     Note added 05/29/16):  Traditional ECLREC has si32 space for     *
*  cell numbers.  Under our spec for 64-bit computing, this is still   *
*  OK (max nelt is 2^31), but Lija.Lijval is currently a long.  So we  *
*  expand the file value if necessary until type of Lija.Lijval gets   *
*  updated.                                                            *
************************************************************************
...
void elijopen(struct CELLTYPE *il, struct RFdef *fptr) {
   ...
   si32 icell;                /* Cell number */
   ui32 iconn;                /* Connection loop counter */
   si32 lij;                  /* Value of Lij sent to comp node */
   si32 ncr;                  /* Cells remaining on current node */
   int node;                  /* Relative destination node */
   ui16 ictyp;                /* Connection type loop index */
            ...
            for (iconn=0; iconn<ix->nc; ) {
               ...
               if ((cld.xicell == icell) && (cld.jct == ictyp)) {
                  ...
                  *  convert Lij to IA offset.  */
                  if (lij < ix->srcnelt)
                     { if (IAinpt) lij += (lij & (-(si32)RP->nsy)); }
                  /* If is skip code, skip this connection */
                  else if (((ulng)lij == ix->lijlim) || (lij == -1))
                     lij = ELIJ_SKIP_ONE;
                  /* Otherwise, external Lij is invalid, terminate */
                  else
                     d3exit(fmturlnm(il), LIJBDV_ERR, ix->ict);
                  /* Send Lij value to corresponding node */
                  nnputi4(&eclnnio, lij);
                  if (Cinput) nnputi2(&eclnnio, cld.ecij);
   ...
   for (;;) {
      ...
      if ((cld.xicell == Lija.kcell) && (cld.jct == Lija.jctype)) {
         ...
         *  If input is from IA, convert Lij to IA offset.  */
            ...
            if (ix->kgfl & KGNIA)
               Lija.lijval += (Lija.lijval & (-(long)RP->nsy));
            else if (Lija.isyn == 0) lijxyr(ix);
            return YES; }
         /* Otherwise, external Lij is invalid, terminate job */
         else d3exit(fmturlnm(Lija.kl), LIJBDV_ERR, (int)ix->ict);
         } /* End processing matching record */
...
#ifdef PARn
   Lija.lijval = (long)nngeti4(&eclnnio);
