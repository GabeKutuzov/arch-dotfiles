/* (c) Copyright 1991-2016, The Rockefeller University *11115* */
/* $Id: d3rlnm.c 70 2017-01-16 19:27:55Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                              d3rlnm.c                                *
*                                                                      *
...
*  R66, 03/16/16, GNR - Move readlid() here from d3g2conn, add scan    *
*                       function, rename to gettpnum(), and call it    *
*                       directly from readrlnm()                       *
...
/*---------------------------------------------------------------------*
*                              gettpnum                                *
*  This little function process the %type field for virtual senses.    *
*  It scans the data field and replaces a '-' with the new generic     *
*  default '1'.  If the source field is VT, the type can be of the     *
*  form arm.joint.  If the index exceeds the maximum number of items   *
*  of its type, an error is generated and the routine sets hbcnm to    *
*  -1 and returns that value (which avoids array overflows in d3tchk). *
*                                                                      *
*  Note:  These inputs are illegal for GCONN cards, but it is harmless *
*  that this code is performed before the caller checks the srctype.   *
*                                                                      *
*  Rev, 03/15/16, GNR - Eliminate global jarm,jwdw,jd1 variables and   *
*                       index incrementing if '-' is coded for index.  *
*---------------------------------------------------------------------*/

static void gettpnum(rlnm *idtyp, int limit, char *name) {

   int index = 1, ijnt = -1;     /* Defaults for hbcnm and hjnt */
   scanck(idtyp->lnm, RK_REQFLD|RK_FENCE, ~(RK_BLANK|RK_COMMA));
   if (!strcmp(idtyp->lnm, "-")) ;
   else if (idtyp->hjnt == VT_SRC) {
      /* Because strchr has no length arg, do our own little search */
      int wid,rwid;
      for (wid=0; wid<LSNAME && idtyp->lnm[wid] != '.'; ++wid) ;
      if (wid > 0)
         wbcdin(idtyp->lnm, &index, RK_INT+RK_QPOS+RK_NINT+wid-1);
      rwid = (LSNAME - 2) - wid;
      if (idtyp->lnm[wid] == '.' && rwid >= 0) {
         wbcdin(idtyp->lnm+wid+1, &ijnt, RK_INT+RK_QPOS+RK_NINT+rwid);
         ijnt -= 1;
         }
      }
   else {
      wbcdin(idtyp->lnm, &index, RK_INT+RK_QPOS+RK_NINT+LSNAME-1);
      }
   if (index > limit) {
      ermark(RK_MARKDLM);
   ...
   idtyp->hbcnm = (si16)index;
   idtyp->hjnt = (si16)ijnt;
   } /* End gettpnum() */
...
*  a positive integer is required.  If an integer, the value is stored *
*  in hbcnm.  If a '-', the action depends on the 'srcid' (region)     *
*  code.  If the srcid is a type that references an arm, window, or    *
*  image touchpad, '1' is coded to reference the new default first     *
*  arm, window, or touch.  Otherwise, '-1' is coded and the caller     *
*  must deal with this default or generate and error.  Anything else   *
*  is an error.  inform() checks for missing fields, bad lengths, and  *
*  bad punctuation.                                                    *
*                                                                      *
*  Arguments:                                                          *
*     srcnm       Ptr to rlnm where results are to be stored.          *
*     il          Ptr to celltype, used to look up names for '*'       *
*                 options.                                             *
*                                                                      *
*  Rev, 04/29/07, GNR - Names in parens are forced to be layer names   *
*  R66, 03/16/16, GNR - Old system of incrementing default arms and    *
*                       windows abolished, just use '1'.  Merge in     *
*                       tests from readlid() routine from d3g2conn.c   *
*                       Read (jntno) for VJ and VT types.              *
*---------------------------------------------------------------------*/
...
int readrlnm(rlnm *srcnm, struct CELLTYPE *il) {
   ...
   /* Determine the input type and temporarily store it in
   *  the hjnt field for gettpnum.  */
   ityp = (RK.scancode & RK_INPARENS) ? REPSRC : getsnsid(srcnm->rnm);
   srcnm->hjnt = (si16)ityp;
   ...
   case REPSRC:
      ...
      break;
   case IA_SRC:
      /* At present we have only one input array, but here would
      *  be the place to select from multiple ones.  */
      gettpnum(srcnm, 1, "INPUT ARRAYS");
      break;
   case VALSRC:
      gettpnum(srcnm, RP->nvblk, "VALUES");
      break;
   case VJ_SRC:
   case VH_SRC:
   case VT_SRC:
      gettpnum(srcnm, (int)RP->narms, "ARMS");
      break;
   case VW_SRC:
   case VS_SRC:
      gettpnum(srcnm, (int)RP->nwdws, "WINDOWS");
      break;
   case TV_SRC:
   case PP_SRC:
   case USRSRC:
      /* These types take a name */
      scanlen(SCANLEN);
      inform("(S,TH" qLTF ")", &srcnm->hbcnm, NULL);
      strncpy(srcnm->lnm, getrktxt(srcnm->hbcnm), LSNAME);
      break;
   case ITPSRC:
      gettpnum(srcnm, (int)RP->nitps, "IMAGE TOUCH PADS");
      break;
      } /* End type switch */
