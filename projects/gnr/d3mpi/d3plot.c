...
/* $Id: d3plot.c 77 2018-03-15 21:08:14Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                              d3plot.c                                *
...
*                                                                      *
*  Value returned:                                                     *
*     A value from the set of OUT_XXX codes defined in rpdef.h which   *
*     caller will store in RP->CP.outnow.  Value indicates whether to  *
*     terminate trial series due to user action detected by newplt.    *
*                                                                      *
************************************************************************
...
*  R70, 01/17/17, GNR - Correct mapping of return codes to OUT_XXX     *
...
/* Superposition plot items done on host only */
      ...
      for (pplb=RP0->pplb1; pplb; pplb=pplb->pnxplb)
         ...
         pencol(pplb->lbcolor);
         symbol(pplb->lbx, pplb->lby, pplb->lht,
      ...
      for (pvc=RP0->pvc1; pvc; pvc=pvc->pnvc)
         if ((pvc->kvp & VKPPL) && ((rc = d3vplt(pvc,
            (si32)RP->CP.trial, pvc->vcplx, pvc->vcply,
            pvc->vcplw, pvc->vcplh)) > 0)) goto PopTimer;
...
/* Perform individual environment plot */
      ...
      if (RP->kpl1 & KPLENV) {
         /* Start plot on all nodes.  d3ngph return does not need
         *  remapping, is already an outnow code.  */
         if ((rc = d3ngph(11.0, 11.0, 0, 0, "DEFAULT",
            RP->colors[CI_BOX], "CLIP", 0)) > 0) goto PopTimer;
#ifndef PARn
...
*  superposition plots, and calls to d3lplt were added.  */
      ...
      if (RP->kpl1 & KPLIRP) for (ir=RP->tree; ir; ir=ir->pnrp) {
         ...
         if (lkrp & (KRPPL|KRPBP) && (lkrp & KRPAP ||
            ...
            if ((rc = d3ngph(8.5, 11.0, 0,0, "DEFAULT",
               RP->colors[CI_BOX], "CLIP", 0)) > 0) goto PopTimer;
            d3rplt(ir, 0, 0.5, 0.5, 7.5, 10.0);
            for (il=ir->play1; il; il=il->play) {
               if (il->Ct.kctp & CTPHP) continue;
               d3lplt(il, il->pps[0]+spsize(il->locell,il->phshft),
                  0, 0, 0);
               }
...
#ifndef PARn
         for (pvc=RP0->pvc1; pvc; pvc=pvc->pnvc)
            /* d3vplt() returns outnow code from d3ngph() */
            if (pvc->kvp & VKPPL && (rc = d3vplt(pvc,
               (si32)RP->CP.trial, 0.5, 0.5, 7.5, 10.0)) > 0)
               goto PopTimer;
#else
         int ivcpl = RP->nvcpl;
         while (ivcpl--) if ((rc = d3ngph(11.0, 11.0, 0, 0,
            "DEFAULT","DEFAULT","CLIP", 0)) > 0) goto PopTimer;
#endif
         } /* End if KPLVCP */
...
*  This will require a changed prototype in d3global.h  */
   ...
   rc = finplt();
   /* Map return code to outnow values */
