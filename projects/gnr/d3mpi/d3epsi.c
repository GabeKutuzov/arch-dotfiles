...
/* $Id: d3epsi.c 70 2017-01-16 19:27:55Z  $ */
/***********************************************************************
*                             CNS Program                              *
*                               d3epsi                                 *
*                                                                      *
...
*  Note: aplx, etc. set to -1 in d3dflt to trigger automatic plots */
   ...
   for (ir=RP->tree; ir!=NULL; ir=ir->pnrp) {
      ...
      ratio = (float)((ui32)ir->ngy*(ui32)ir->nqy) /
              (float)((ui32)ir->ngx*(ui32)ir->nqx);
...
*  because option may be turned on at d3chng time.  */
   ...
   for (ir=RP->tree; ir!=NULL; ir=ir->pnrp) {
      if (ir->Rp.vxo==0.0 && ir->Rp.vyo==0.0)
         ir->Rp.vyo = -(ir->aplh + 1.5*RP->stdlht);
      } /* End KP=V loop */
