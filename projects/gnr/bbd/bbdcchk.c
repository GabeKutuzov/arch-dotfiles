/* (c) Copyright 2007-2016, The Rockefeller University *11116* */
/* $Id: bbdcchk.c 28 2017-01-13 20:35:15Z  $ */
/***********************************************************************
*                      BBD (Client Side) Package                       *
*                              bbdcchk.c                               *
*           Check setup of a BBD with a network simulation             *
*                                                                      *
*  This routine must be called from a BBD (if using the package)       *
*  after all cameras, senses, values, and effectors have been          *
*  registered in order to validate the setup with the network          *
*  simulator (presumably CNS) and finalize the data structures         *
*  that will be used during the actual simulation.  The server         *
*  side must call bbdschk() to communicate with this routine.          *
*                                                                      *
*  Synopsis:                                                           *
*     int bbdcchk(void)                                                *
*                                                                      *
...
*     be accessible in global memory via the BBDcd structure.          *
*                                                                      *
*  Return value:                                                       *
...
*  ==>, 08/12/07, GNR - Last mod before committing to svn repository   *
*  Rev, 09/26/08, GNR - Minor type changes for 64-bit compilation      *
*  V1C, 11/11/10, GNR - Better error checking for bad child procs.     *
...
*                                                                      *
*  (Argument is pointer to pointer to both original setup list and     *
*   start of sorted device list.)                                      *
*=====================================================================*/

static void bbdcclst(BBDDev **ppsdev) {
   ...
   *  (from server's point of view).  */
   while (len = rfri4(BBDcd.ndrecv)) { /* Assignment intended */
...
#ifdef DEBUG
      fputs(ssprintf(NULL, "  Read name <%15s> from server\n", dname),
         stderr);
#endif
      ...
      fputs(ssprintf(NULL, "Name from server <%15s> not found "
         "on BBD.\n", dname), stderr);
      nack -= 1;
      continue;
...
GotNameMatch:
#ifdef DEBUG
      fputs(ssprintf(NULL, "  Read length %d\n", len), stderr);
#endif
      if (len != pd->ldat) {
         fputs(ssprintf(NULL,
            "Length mismatch: %6ld != %6ld for %15s\n",
            (long)len, (long)pd->ldat, pd->devnm), stderr);
         nack -= 1; }
      /* Read the flags.  Everything should match except 0xf0 bits.
      *  Set BBDUsed bit from server so client can skip sending data
      *  for unused devices.  */
      rdflgs = rfri2(BBDcd.ndrecv);
      if ((rdflgs ^ pd->bdflgs) & ~BBDUMask) {
         fputs(ssprintf(NULL, "Flags mismatch: %6x != %6x for %15s\n",
            (int)rdflgs, (int)pd->bdflgs, pd->devnm), stderr);
         nack -= 1; }
...
#ifdef DEBUG
      fputs(ssprintf(NULL, "  Read bdid %d\n", (int)pd->bdid), stderr);
#endif
      pack += 1;
   ...
   for (pd=pd0; pd; pd=pd->pndev) {
      fputs(ssprintf(NULL, "Name from BBD <%15s> not found "
         "on server.\n", pd->devnm), stderr);
      nack -= 1;
      }
   if (nack) pack = nack;
...
#endif
   short  initcode;           /* Initial handshake code */
...
#ifdef DEBUG
   fputs("-->bbdcchk processing sensors\n", stderr);
#endif
   if (BBDcd.pfsns) bbdcclst(&BBDcd.pfsns);
#ifdef DEBUG
   fputs("-->bbdcchk processing ttvs\n", stderr);
#endif
   if (BBDcd.pfttv) bbdcclst(&BBDcd.pfttv);
#ifdef DEBUG
   fputs("-->bbdcchk processing etvs\n", stderr);
#endif
   if (BBDcd.pfetv) bbdcclst(&BBDcd.pfetv);
#ifdef DEBUG
   fputs("-->bbdcchk processing stvs\n", stderr);
#endif
   if (BBDcd.pfstv) bbdcclst(&BBDcd.pfstv);
#ifdef DEBUG
   fputs("-->bbdcchk processing values\n", stderr);
#endif
   if (BBDcd.pfval) bbdcclst(&BBDcd.pfval);
#ifdef DEBUG
   fputs("-->bbdcchk processing effectors\n", stderr);
#endif
   if (BBDcd.pfeff) bbdcclst(&BBDcd.pfeff);
#ifdef DEBUG
   fputs("-->bbdcchk returning\n", stderr);
#endif
