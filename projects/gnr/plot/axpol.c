...
/* $Id: axpol.c 20 2011-12-24 02:59:03Z  $ */
/***********************************************************************
*                               axpol()                                *
*                                                                      *
*  SYNOPSIS:                                                           *
...
void axpol(float x, float y, const char *label, float radius,
   ...
   if (_RKG.pcw->MFActive && radius > 0.0) {
...
DoneMinorTicks:
            ...
            if (dovlbls) {       /* Plot long spoke angle labels */
               ...
               bcdout(RK_D+RK_D+RK_IORF+RK_SNGL+RK_LFTJ+MXTVLEN-1,
                  printme, tval);
               nn = RK.length + (printme[RK.length] == '0' ? -1 : 1);
               hlblwd = (float)nn*ASPECT*hlblht;
