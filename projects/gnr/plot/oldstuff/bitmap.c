...
*     plotted.  Data for each horizontal row are packed in left-to-    *
*     right order, with one bit per pixel if the 'type' is BM_BW, one  *
*     byte per pixel if the 'type' is BM_GS or BM_C8, two bytes per    *
*     pixel if the 'type' is BM_C16, three bytes per pixel if the      *
*     'type' is BM_C24, otherwise 'lci' bits per pixel (i.e. each      *
*     value is a color index) if the type is BM_COLOR.  Data for each  *
*     new row begin on a byte boundary.  Rows are arranged in memory   *
...
*     BM_GS (1) if the bitmap contains grayscale data, BM_COLOR (4)    *
*     if the bitmap contains color index data, BM_C8 if the bitmap     *
*     contains 8-bit color data (2-3-3 BGR), BM_C16 if the bitmap      *
*     contains 16-bit color data (5-5-5 RGB), or BM_C24 if the bitmap  *
*     contains 24-bit color data (8-8-8 RGB).  Add BM_NSME (8) to      *
*     this value if submap extraction is not to be performed on the    *
*     'array' argument.  Other 'type' values may be defined in         *
*     future versions of this library to specify bitmap data           *
*     compression.                                                     *
*                                                                      *
...
*     Color data can be provided in BM_C8, BM_C16, or BM_C24 modes.    *
*                                                                      *
