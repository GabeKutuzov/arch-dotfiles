...
*  ==>, 10/07/15, GNR - Last date before committing to svn repository  *
   Rev, 12/26/18, GMK - CHANGE HERE: CHECK FOR MIN BUFF SIZE           *
   Rev, 01/16/18, GMK - Added 'additional header information'          *
***********************************************************************/
...
#define LEN_TITL_REC  60
#define LEN_TIME_REC  13

void mfhead(void) {

   Win *pwin = _RKG.pcw;
   int lrec,ltit;             /* Bit,byte lengths of hdr records */

   /* Signal that this record contains headers */
   pwin->MFCountWord |= MFB_INIT;
   
   /* Perform mfhchk for all record lengths */
   mfhchk(Bytes2Bits(LEN_VERS_REC+LEN_TITL_REC+
      LEN_TIME_REC+mxlNNode));

   /* Version record */
   lrec = Bytes2Bits(LEN_VERS_REC);
   ...
   *pwin->MFCP++ = '\n';
   ...
   /* Time stamp record */
   lrec = Bytes2Bits(LEN_TIME_REC);
   tstamp((char *)pwin->MFCP);
   pwin->MFCP[LEN_TIME_REC-1] = '\n';
   pwin->MFCP += LEN_TIME_REC;
   pwin->MFBrem -= lrec;

   /* Additional header information record */
   c2mf(OpNNode);
   mfbitpk(AHIV,4);
   mfbitpk(_RKG.s.numnodes,16);
