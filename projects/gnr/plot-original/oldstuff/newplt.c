...
/* $Id: newplt.c 29 2017-01-03 22:07:40Z  $ */
/***********************************************************************
*                              newplt()                                *
*                                                                      *
* SYNOPSIS:                                                            *
...
* RETURN VALUES:                                                       *
*  0        Normal completion                                          *
*  1        Interrupt button pushed, indicating user termination rqst. *
*           Note: Return code is broadcast to all nodes internally.    *
*  Other    Error condition--currently none defined                    *
*                                                                      *
* REMARKS:                                                             *
...
int newplt(float xsiz, float ysiz, float xorg, float yorg,
   ...
   if (_RKG.MFFlags & MFF_MEnded) abexit(261);   /* No msg, not likely */
...
*  output and do an outgoing synch before writing any stuff from
...
*  returned from host window if one exists.  */
   ...
   if (_RKG.MFFlags & MFF_FramePend) {
      ...
      dbgprt(ssprintf(NULL, "newplt called bpause with msgbutt = %4m",
         &_RKG.msgbutt));
#endif
      _RKG.MFFlags &= ~(MFF_SentCOMPLT|MFF_FramePend);
#ifndef PARn
...
#ifndef PARn
   _RKG.s.MFActive = rc ? 0 : (_RKG.s.MFMode & ~kout);
   if (_RKG.s.MFActive) {
      ...
      *  is active--before following broadcst allows comp nodes
...
*  nodes to start drawing.  */
   ...
   if (IAmMulti) {            /* Must be PAR at run time */
      ...
      char crc[FMJSIZE];
#ifdef PAR0
      bcodes = _RKG.s.MFActive | rc;
      lemfmi4(crc, bcodes);
#if defined(DEBUG) && DEBUG & MFDBG_NWPLT
      dbgprt(ssprintf(NULL, "newplt about to broadcast bcodes = %jd",
         bcodes));
#endif
#endif
#ifdef USE_MPI
...
#ifdef PARn
      bcodes = lemtoi4(crc);
      rc = bcodes & (BUT_INTX|BUT_QUIT);
#if defined(DEBUG) && DEBUG & MFDBG_NWPLT
      dbgprt(ssprintf(NULL, "Received bcodes = %jd", bcodes));
#endif
      _RKG.s.MFActive = bcodes & (METF|SGX);
#endif
      } /* End if multinode */
#endif
   if (_RKG.s.MFActive) _RKG.MFFlags |= MFF_FramePend;
