...
/* $Id: upks1s.c 9 2016-01-04 19:30:24Z  $ */
/***********************************************************************
...
*                         upks1sd and upks1sf                          *
*                                                                      *
*  These functions compute an "unequal-partitioned" approximation to   *
*  the one-sided multidimensional Kolmogorov-Smirnov statistic.  The   *
*  MDKS may be used to quantitate the degree to which two distribu-    *
*  tions of points in k-dimensional space are statistically distin-    *
*  guishable.  This version allocates points in "bricks" that overlap  *
*  the current test point among all quadrants that so overlap in pro-  *
*  protion to the fractional position of the test point in its brick.  *
*  Slower versions that exactly compute the version of the statistic   *
*  defined in the Fasano and Franceschini reference given below are    *
*  named mdks1sd and mkds1sf, respectively.                            *
*                                                                      *
...
*                                                                      *
*  MATLAB Synopses:                                                    *
*     s = upks1sx(MDKSCOM, X1, X2)                                     *
*  C Synopses:                                                         *
*     double upks1sx(void *pwk, Xdat const *X1, Xdat const *X2)        *
*                                                                      *
*  Arguments:                                                          *
*     X1,X2    Pointers to the sets of points to be compared.  X1 and  *
*              X2 must be arrays of whatever type the upks1sx version  *
...
*                                                                      *
*  Return values:                                                      *
*     upks1sx() returns an approximation to the one-sided MDKS statis- *
...
*  here, the number of points in any brick that overlaps the current   *
*  test point are distributed among the quadrants with overlapping     *
*  coordinates in proportion to the fractional position of the test    *
*  point in its brick.  For large numbers of equally distributed data  *
*  points, on average this will give small differences in the quadrant *
*  counts and the statistic will be approximately correct.             *
*                                                                      *
*  Notes:                                                              *
...
*  X2, multiplied by sqrt(n1*n2/(n1+n2)).  This is done by the upks2sx *
*  routines.                                                           *
...
*  for both argument orders by the upks2sx() code.                     *
*                                                                      *
*  Reference:                                                          *
...
************************************************************************
*  V7A, 08/23/15, G.N. Reeke - New program, modfied from mdks1s        *
*  ==>, 08/25/15, GNR - Last date before committing to svn repository  *
...
*  makefile.  If compiled without external definitions, upks1s will    *
*  default to double-precision data with integer ni and name 'upks1s'. *
*  (Appropriate typedefs are in the header file mdksint.h)             *
*=====================================================================*/

#ifdef I_AM_MATLAB
#define MyName vfnmex("upks1s",VL)
#define printf mexPrintf

enum rhsargs { jwk, jX1, jX2 };

/*=====================================================================*
*                    upks1sx mex function wrapper                      *
*=====================================================================*/
...
/* Perform the statistical computation and return the result */
   ...
   s = vfnname(upks1s,VL)(pwk, pX1, pX2);
...
static void vfnname(upkssrt,VL)(Wkhd *phd, int jx, int jk) {
...
double vfnname(upksqd,VL)(Wkhd *phd, int ix1) {

   double upks = 0.0;         /* The result */
...
*  member list pointers, so treat Link list as an array.  */
   ...
   printf(" \n");
         ...
         /* Store in the Qfr0 array the fractional distance of this
         *  test point from the two edges of its ribbon in each
         *  coordinate.  */
         {  Ribn *prib, *prib1 = phd->pRib0;
            Prec *pfrk = phd->pQfr0;
            size_t mask0 = phd->pDim0->mask;
            size_t oq,qq;              /* Output bits */
            int jk;                    /* Dimension index */
            int nrpd = 1 << phd->d;    /* Ribbons per dim */
            for (jk=0; jk<k; prib1+=nrpd,++jk) {
               /* Find correct ribbon for this dimension */
               qq = 0, oq = (size_t)1;
               osubj = (ptl1->obrk >> jk) & mask0;
               while (osubj) {
                  if (osubj & (size_t)1) qq |= oq;
                  oq += oq;
                  osubj >>= k;
                  }
               prib = prib1 + qq;
               pfrk[0] = prib->rdiff * (pX1[jk] - prib->qmin);
               pfrk[1] = 1.0 - pfrk[0];
               pfrk += NENDS;
               }
            } /* End prib,pfrk, etc. local scope */
...
NewLevel:   osubj = ptl1->obrk >> lshft & nsmask;
   ...
   printf("      No overlap, new quad %zd total is %.3f\n",
      qo, pqtot[qo]);
               ...
               *  there is coordinate overlap according to the frac-
               *  tional position of the test point in the brick.  If
               *  all coordinates overlap, this brick overlaps the
               *  test point, so subtract one from the difference for
               *  the edge overlap.  In the upks approximation, there
               *  is no way to catch overlaps in coordinates of other
               *  data points (which should not happen with continuous
               *  data).  */
               size_t iqo,magic;    /* For loop over qn bits */
               Prec np2a;           /* Points to be allocated */
               Prec npq;            /* Points allocated to quad */

               np2a = (Prec)(pbrkj[ochld].npts - (qn == nsmask));
               magic = (~qn + 1);

#if MD1SDBG & MDBG_OVLP
   printf("      Overlap at bottom level, qn = &zd, np2a = %.3f\n",
      qn, np2a);
#endif
               ...
               do {
                  ...
                  size_t jjq;
                  npq = np2a;
                  for (jjq=1; plvl->relqn>=jjq; jjq<<=1) {
                     if (plvl->relqn & jjq)
                        npq *= pfrk[(jjq & iqo) != 0];
                     } /* End loop over coordinate dims */
                  pqtot[qo|iqo] += npq;

#if MD1SDBG & MDBG_BOTL
   printf("         Adding %.3f to quad %zd for brick %zd:\n",
      npq,qo|iqo,ochld);
   printf("         New quad total is %.3f\n", pqtot[qo|iqo]);
#endif
                  iqo = (iqo + magic) & qn;
                  } while (iqo);
               }
...
double vfnname(upks1s,VL)(void *pwk, Xdat const *X1, Xdat const *X2) {
...
*  sorted data to determine which brick each point belongs to.  */
      ...
      for (jk=0; jk<k; ++pdim,++jk) {
         ...
         vfnname(upkssrt,VL)(phd, IXb, jk);

         /* Scan the sorted list of data points, dividing it into 2^td
         *  regions with ndpd+1 points in the first rdiv regions and
         *  ndpd points in the rest.  Accumulate the ribbon information
         *  (by dimension (fast) and brick number (slow) in the pRib
         *  array and the obrk field in each Link.  */
         ptl = phd->pLnk1[IXb];
         jjd = (size_t)1 << td;
         jdiv = rdiv;
         while (jjd-- > 0) {
            ...
            Xdat qdiff;
            prib->qmin = ptl->pdat[jk];
            idiv = ndpd + (jdiv-- > 0);
            while (idiv-- > 0) {
               ptl->obrk |= obrtd;
               ptl = (ppl = ptl)->pnxt;
               } /* End assignment to one brick */
            qdiff = ppl->pdat[jk] - prib->qmin;
            prib->rdiff = (qdiff > TINY) ? 1.0/qdiff : 0;   /* JIC */
            ++prib;           /* On to next ribbon */
...
*  the algorithm in upksqdx can skip empty bricks.  */
...
*---------------------------------------------------------------------*/
   ...
   return vfnname(upksqd,VL)(phd, 0);
