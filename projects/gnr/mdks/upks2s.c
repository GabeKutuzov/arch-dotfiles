...
/* $Id: upks2s.c 8 2015-11-25 20:04:01Z  $ */
/***********************************************************************
...
*  along with pointers to the two data sets to be compared.  upks2sx() *
...
*                                                                      *
*  MATLAB Synopsis:                                                    *
*     s = upks2sx(MDSKCOM, X1, X2)                                     *
*  C or C++ Synopsis:                                                  *
*     double upks2sx(void *pwk, Xdat const *X1, Xdat const *X2)        *
*                                                                      *
*  Arguments:                                                          *
...
*  coordinates in proportion tothe fractional position of the test     *
...
************************************************************************
*  V7A, 07/18/15, G.N. Reeke - New program, modfied from mdks2s        *
...
*  makefile.  If compiled without external definitions, upks2s will    *
*  default to double-precision data with integer ni and name 'upks2s'. *
*  (This indirect method is used because #if cannot test strings.)     *
*=====================================================================*/

#ifdef I_AM_MATLAB
#define MyName vfnmex("upks2s",VL)

enum rhss2args { jwk, jX1, jX2 };

/*=====================================================================*
*                    upks2sx mex function wrapper                      *
*=====================================================================*/
...
/* Perform the statistical computation and return the result */
   ...
   s = vfnname(upks2s,VL)(pwk, pX1, pX2);
...
double vfnname(upks2s,VL)(void *pwk, Xdat const *X1, Xdat const *X2) {
   ...
   if ((s1 = vfnname(upks1s,VL)(pwk, X1, X2)) <= 0.0) return s1;
   if ((s2 = vfnname(upksqd,VL)(pwk, IX2)) <= 0.0) return s1;
