...
/* $Id: mdks1s.c 6 2014-06-09 19:19:58Z  $ */
/***********************************************************************
 ...
 *  ple calls to mdks1sx() that have the same ni, gi, and k parameters. *
 ...
 *              1 for 'double').  X1 has dimension n1 x k and X2 has    *
 *              dimension n2 x k.  The coordinates are assumed to be    *
 *              stored in the native byte order of the machine on which *
 *              the program runs.                                       *
 *     pwk      This must be the pointer stored at *ppwk by a call to   *
 *              mdksallox() with the 'ni', 'gi', and 'k' of X1 and X2.  *
 ...
 *  test, X1 should be the experimental data and X2 the model.  A       *
 *  negative value is returned in case of an error as follows:          *
...
#define MDBG_DIFF  128  /* Print quad differences */
...
#ifdef I_AM_MATLAB
#define MyName vfnmex("ank_mdks1s",VL)
...
void mexFunction(int nlhs, mxArray *plhs[],
  ...
  s = vfnname(ank_mdks1s,VL)(pwk, pX1, pX2);
...
static void vfnname(mdkssrt,VL)(Wkhd *phd, int jx, int jk) {
  ...
  for (ii=0; ii<=isz; ++ii) {
    ...
    for (ptl=phd->pLnk1[jx]; ptl; ptl=ptl->pnxt) {
      pb = pb0 + ((byte const *)(ptl->pdat+jk))[i];
      pb->tail->pnxt = ptl;
      pb->tail = ptl;
    } /* End loop over data records */

      /* End of pass, stack up the bins in order.  Stack any
       *  negatives first when stacking the high order.  */
    ppl = &phd->pLnk1[jx];
    if (ii == isz) {        /* High order byte w/sign */
      for (pb=pbhe; pb<pbe; ++pb) if (pb->head) {
	  *ppl = pb->head;
	  ppl = &pb->tail->pnxt; }
      for (pb=pb0; pb<pbhe; ++pb) if (pb->head) {
	  *ppl = pb->head;
	  ppl = &pb->tail->pnxt; }
    }
    else {                  /* Not high-order byte */
...
double vfnname(mdksqd,VL)(Wkhd *phd, int ix1) {
  ...
  if (k == 2) {
    ...
    /*---------------------------------------------------------------------*
     *  Calculation of MDKS for k > 1.  This is the V6 algorithm:  Loop     *
     *  over all points in the X1 set.  For each point, zero both sets of   *
     *  quadrant counts.  Starting at level 1, loop over all cubes at the   *
     *  current level.  Check whether any of the coordinate bits of this    *
     *  cube overlap the coordinate bits of the current source point.  If   *
     *  there is no overlap, add the number of points in the target cube    *
     *  into the proper quad sum.  If there is overlap, the action depends  *
     *  on what is the current level:  if at the bottom level, loop over    *
     *  the data points that are members of the current target cube, find   *
     *  out the relationship of each target point to the source point and   *
     *  add one to the appropriate quadrant sum.  If not at the bottom      *
     *  level, descend one level and perform the octant scan there.         *
     *  Finally, find the maximum difference in quadrant counts and set     *
     *  the result statistic to the global maximum of this difference.      *
     *  The Bin space is reused to keep track of the quadrant counts.       *
     *---------------------------------------------------------------------*/

    Link *ptl1,*ptl1e,*ptl2;  
    ORibn *poribn = phd->pORibn_dat;
    Prec *pnq1,*pnq2;
    int xtd = 1 << phd->d;
    size_t lqds = (size_t)sizeof(Prec) << (k+1);
    ...
    Prec ld, rd, ur, dr, lu, ru, ul, dl;
    Prec ribct[4];
    size_t szrb = (size_t)(sizeof(Prec) * 4);     
    ...
    for ( ; ptl1<ptl1e; ++ptl1) {
      ...
      for (ik = 0; ik < k; ++ ik)
	{	 for (irib = 0; irib < xtd; ++ irib)
	    {	 if (poribn[irib].krib[(ik ^ 1)].qmax >= ptl1->pdat[(ik ^ 1)])
		break;
	    }
	  if (ik)
	    xrib = irib;
      ...
      gapxlength = (poribn[xrib].krib[0].qmax) - (ptl1->pdat[0]); 
      right = gapxlength / (poribn[xrib].krib[0].qdiff); 
      left = 1 - right;
      gapylength = (poribn[yrib].krib[1].qmax) - (ptl1->pdat[1]); 
      down = gapylength / (poribn[yrib].krib[1].qdiff); 
      up = 1 - down;
	...
	for (jq=plvl->jcube; jq<nq; ++jq) {
	  ...
	  if (!qn) {
	    ...
	    printf("      No overlap, new quad %zd total is %d\n",
		   qo, pqtot[qo]);
#endif
	    continue;
	  }
	...
	/* If the current level is also the lowest level, begin approximation algorithm,
	   which assumes that within bricks the data is uniformly distributed, and therefore
	   uses a geometric approach to assign bricks to quadrants.
	*/
	...
	mybrk = ptl1->obrk & pdim[1].mask;
	if (poribn[yrib].krib[1].qdiff)
	  {	ybrk = mybrk & pdim[1].mask;
	    xbrk = mybrk & pdim[0].mask;
	    while (mybrk < ptl1->obrk) 
	      {	lsum += pbrkj[mybrk].npts;
		xbrk = mybrk & pdim[0].mask;
		xbrk = (xbrk + pdim[0].oadd) & pdim[0].mask;
		mybrk = xbrk | ybrk;
	      }
	    if (mybrk == ptl1->obrk)
	      {	xbrk = mybrk & pdim[0].mask;
	    ...
	    while (xbrk != 0)
	      {	rsum += pbrkj[mybrk].npts;
		xbrk = mybrk & pdim[0].mask;
		xbrk = (xbrk + pdim[0].oadd) & pdim[0].mask;
		mybrk = xbrk | ybrk;	
	      }
#if MD1SDBG & MDBG_BOTL
	    printf ("Horizontal overlap\n");
	    for (kk=0; kk<4; ++kk) printf("    New quad %d total is %.5f\n", kk, pqtot[kk]);
#endif
	  }
	mybrk = ptl1->obrk & pdim[0].mask;
	if (poribn[xrib].krib[0].qdiff)
	  {	xbrk = mybrk & pdim[0].mask;
	    ybrk = mybrk & pdim[1].mask;
	    while (mybrk < ptl1->obrk) 
	      {	usum += pbrkj[mybrk].npts;
		ybrk = mybrk & pdim[1].mask;
		ybrk = (ybrk + pdim[1].oadd) & pdim[1].mask;
		mybrk = ybrk | xbrk;
	      }
	    if (mybrk == ptl1->obrk)
	      {	ybrk = mybrk & pdim[1].mask;
	    ...
	    while (ybrk != 0) 
	      {	dsum += pbrkj[mybrk].npts;
		ybrk = mybrk & pdim[1].mask;
		ybrk = (ybrk + pdim[1].oadd) & pdim[1].mask;
		mybrk = ybrk | xbrk;
	      }
#if MD1SDBG & MDBG_BOTL
	    printf ("Vertical overlap\n");
	    for (kk=0; kk<4; ++kk) printf("    New quad %d total is %.5f\n", kk, pqtot[kk]);
#endif
	  }
...
#if MD1SDBG & MDBG_EXPT
	if (jx == 0)	
	  {	printf("           \nQuad 0, data set 1: %.4f = stuff + %.4f\n", pnq1[0], ribct[0]);
	    printf("     Quad 1, data set 1: %.4f = stuff + %.4f\n", pnq1[1], ribct[1]);
	    printf("     Quad 2, data set 1: %.4f = stuff + %.4f\n", pnq1[2], ribct[2]);
	    printf("     Quad 3, data set 1: %.4f = stuff + %.4f\n", pnq1[3], ribct[3]);
	  }
	else
	  {	printf("           \nQuad 0, data set 2: %.4f = stuff + %.4f\n", pnq2[0], ribct[0]);
	    printf("     Quad 1, data set 2: %.4f = stuff + %.4f\n", pnq2[1], ribct[1]);
	    printf("     Quad 2, data set 2: %.4f = stuff + %.4f\n", pnq2[2], ribct[2]);
	    printf("     Quad 3, data set 2: %.4f = stuff + %.4f\n", pnq2[3], ribct[3]);
	  }
#endif    
      ...
      for (jq=0; jq<nq; ++jq) {
	dnq = fabs((double)pnq1[jq] - nrat*(double)pnq2[jq]);

#if MD1SDBG & MDBG_DIFF	      
	printf("  Adjusted quad %zd difference is %f\n", jq, dnq);
#endif
	if (dnq > mdks) mdks = dnq;
...
double vfnname(ank_mdks1s,VL)(void *pwk, Xdat const *X1, Xdat const *X2) {
  ...
  if (k == 2) {
    ...
    memset(plvl0->pbrk1[IXb], 0, phd->lbrks);
    ...
    for (jk=0; jk<k; ++pdim,++jk) {
      ...
      Prec mincoord;        /* Variable to store minimum coordinate */
      int ribcount;
      ...
      ribcount = 0;
      while (jjd-- > 0) {
	Link *ppl;  /* Ptr to previous link */
	idiv = ndpd + (jdiv-- > 0);
	mincoord = ptl->pdat[jk]; /* Here we store the min ribbon coordinate */  	while (idiv-- > 0) {
	  ptl->obrk |= obrtd;
	  ppl = ptl;
	  ptl = ptl->pnxt;
	} /* End assignment to one brick */
	poribn1[ribcount].krib[jk].qmax = ppl->pdat[jk]; /* Now ppl->pdat[jk] gives highest coord in this ribbon */
	poribn1[ribcount].krib[jk].qdiff = poribn1[ribcount].krib[jk].qmax - mincoord;
	++ ribcount;
	obrtd = (obrtd + pdim->oadd) & pdim->mask;
      } /* End loop over axial divisions */
    } /* End loop over dimensions */

    /* Stage II.  Now working separately on the two data sets, at each
     *  brick make a list of the data points that belong to that brick
     *  and count them.  Because the linked list now spans the merged
     *  data, we just loop over the Links treated as arrays.  */
	
    ptl = phd->pLnk0[IXb];
    for (jx=0; jx<NDSET; ++jx) {
      Link **ppbm0 = phd->ppBm0[jx];
      Brick *pbrkj = (plvl = plvle)->pbrk1[jx];
	...
	*ppbr = ptl;     
	...
	for (ibrk=0; ibrk<nbrks; ++ibrk) {
       ...
       *  the algorithm in mdksqdx can skip empty bricks.  */
	...
	if (pinob < pdown) pinob->inob = nq;
      ...
      for (ii=1; ii<=td; ++ii) {
	Brick *pbr = phd->pLvl0[ii-1].pbrk1[jx];
	size_t ibrk, nbrks = (size_t)1 << ii*k;
	printf("\n==>Level %d brick counts for set %d\n", ii, jx);
	for (ibrk=0; ibrk<nbrks; ++ibrk) {
	  Nint bct = pbr[ibrk].npts;
	  printf("  At");
	  dbgqk(phd, ibrk);
#if KTN == 0
	  printf(" %d, inob = %d\n", bct, pbr[ibrk].inob);
